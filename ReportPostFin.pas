...
begin
   ...
   case vid.itemindex of
     ...
     1: begin
           ...
           Title:='Финансовые показатели продаж по покупателям за период с '+date_sokr(dt1.date)+' по '+date_sokr(dt2.date);
           TITLE2:='Покупатель';
        end;
     2: begin
           SQL:='select * from report_new_shop_1(:dt1,:dt2,:shop, :RNTYPS, :TWTREE_ID, :TYP, :KLNID, :TYP_KLN, :POSTID, :POST_TYP, :RNS_TABEL, :DIRECT_POST) ';
           Title:='Финансовые показатели продаж по магазинам за период с '+date_sokr(dt1.date)+' по '+date_sokr(dt2.date);
           TITLE2:='магазин';
        end;
     3: begin
           SQL:='select * from report_new_rntyp_1(:dt1,:dt2,:shop, :RNTYPS, :TWTREE_ID, :TYP, :KLNID, :TYP_KLN, :POSTID, :POST_TYP, :RNS_TABEL, :DIRECT_POST) ';
           Title:='Финансовые показатели продаж по типам расходных накладных за период с '+date_sokr(dt1.date)+' по '+date_sokr(dt2.date);
