...
uses
  ...
  ppTypes, ppUtils, ppTreeVw, daQClass, ComCtrls;
...
begin
  ...
  if (FlvwJoins.Items.Count = 0) then
    begin
      {'Join fields have not been selected.'}
      ShowMessage(ppLoadStr(780));
...
begin
  if (FlvwJoins.Selected <> nil) then
    begin
      ...
      cbxOperators.Top := FlvwJoins.Selected.Top + FlvwJoins.Selected.Top;
...
begin
  ...
  if (FTable1 <> nil) then
    FlvwJoins.Columns[2].Caption := FTable1.TableAlias + ' ' + ppLoadStr(760) {Field}
  else
    FlvwJoins.Columns[2].Caption := ppLoadStr(760); {Field}
