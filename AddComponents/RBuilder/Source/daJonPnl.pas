...
uses
  ...
  ppComm, ppTypes, ppUtils, ppTreeVw, daQClass, daDatMan;
...
begin
  ...
  FSQL.CreateFieldsForTable(aTable, lFields);
  ...
  for liIndex := 0 to lFields.Count - 1 do
    begin
      ...
      lItem.Caption := lField.FieldName;

      lsDataType := daDataTypeToString(lField.DataType);
...
begin
  ...
  lField1 := FSQL.GetFieldForTable(FTable1, aField1);
  lField1.ChildType := Ord(dactForeignField);
  lField1.Parent := lTableJoin;

  lField2 := FSQL.GetFieldForTable(FTable2, aField2);
