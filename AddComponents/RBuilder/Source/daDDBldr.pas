{******************************************************************************}
{                                                                              }
{                   ReportBuilder Data Dictionary Builder                      }
{                                                                              }
...
uses
  ...
  StdCtrls, ComCtrls, ExtCtrls, Buttons, Menus, IniFiles,
  ppComm, ppClass, ppTreeVw, ppDB, ppTypes, ppUtils, ppDsgnDB, ppForms, ppDBPipe,
  daDB, daDataVw, daQuery, daQClass, ppDsIntf, daJonPnl;
...
type
  ...
  TdaDataDictionaryBuilderForm = class(TppComponentDesigner)
    ...
    private
      ...
      procedure LoadPageStatesFromIniFile;
  ...
  TdaDDListViewPage = class(TdaDDEditPage)
    ...
    public
      ...
      procedure LoadFromIniFile(aIniFile: TIniFile);
      procedure SaveToIniFile(aIniFile: TIniFile);
...
{------------------------------------------------------------------------------}
{ TdaDataDictionaryBuilderForm.LoadFromIniFile }

procedure TdaDataDictionaryBuilderForm.LoadFromIniFile;
var
  lIniFile: TIniFile;
  liLeft: Integer;
  liTop: Integer;
...
begin
  ...
  liWindowState := lIniFile.ReadInteger('Data Dictionary Builder', 'WindowState', Ord(wsNormal));

  if TWindowState(liWindowState) = wsNormal then
    begin
      liLeft   := lIniFile.ReadInteger('Data Dictionary Builder', 'Left',   Left);
      liTop    := lIniFile.ReadInteger('Data Dictionary Builder', 'Top',    Top);
      liWidth  := lIniFile.ReadInteger('Data Dictionary Builder', 'Width',  Width);
      liHeight := lIniFile.ReadInteger('Data Dictionary Builder', 'Height', Height);
...
{------------------------------------------------------------------------------}
{ TdaDataDictionaryBuilderForm.LoadPageStatesFromIniFile }

procedure TdaDataDictionaryBuilderForm.LoadPageStatesFromIniFile;
var
  lIniFile: TIniFile;
  liIndex: Integer;
...
begin
  ...
  if (FActivePage = -1) then
    begin
      FActivePage := lIniFile.ReadInteger('Data Dictionary Builder', 'Active Page', 0);
  ...
  for liIndex := 0 to FPages.Count - 1 do
    begin
      ...
      lPage.LoadFromIniFile(lIniFile);
    end;

  lIniFile.Free;

end; {procedure, LoadPageStatesFromIniFile}

{------------------------------------------------------------------------------}
{ TdaDataDictionaryBuilderForm.SaveToIniFile }

procedure TdaDataDictionaryBuilderForm.SaveToIniFile;
var
  lIniFile: TIniFile;
  liIndex: Integer;
  lPage: TdaDDListViewPage;
begin
  ...
  lIniFile.WriteInteger('Data Dictionary Builder', 'WindowState', Ord(WindowState));
  lIniFile.WriteInteger('Data Dictionary Builder', 'Left',   Left);
  lIniFile.WriteInteger('Data Dictionary Builder', 'Top',    Top);
  lIniFile.WriteInteger('Data Dictionary Builder', 'Width',  Width);
  lIniFile.WriteInteger('Data Dictionary Builder', 'Height', Height);

  lIniFile.WriteInteger('Data Dictionary Builder', 'Active Page', pgcMain.ActivePage.TabIndex);

  for liIndex := 0 to FPages.Count - 1 do
    begin
      lPage := TdaDDListViewPage(FPages[liIndex]);

      lPage.SaveToIniFile(lIniFile);
    end;
...
{------------------------------------------------------------------------------}
{ TdaDDListViewPage.LoadFromIniFile }

procedure TdaDDListViewPage.LoadFromIniFile(aIniFile: TIniFile);
var
  lsColumnName: String;
  liIndex: Integer;
begin

  FSortMode := aIniFile.ReadInteger('Data Dictionary Builder', PageName + ' Sort Mode', 1);
  ...
  for liIndex := 0 to FFieldNames.Count - 1 do
    begin
      ...
      FColumnSizes.Add(TObject(aIniFile.ReadInteger('Data Dictionary Builder', lsColumnName, 200)));
    end;

end; {procedure, LoadFromIniFile}

{------------------------------------------------------------------------------}
{ TdaDDListViewPage.SaveToIniFile }

procedure TdaDDListViewPage.SaveToIniFile(aIniFile: TIniFile);
var
  lsColumnName: String;
  liIndex: Integer;
begin
  ...
  aIniFile.WriteInteger('Data Dictionary Builder', PageName + ' Sort Mode', FSortMode);

  for liIndex := 0 to FFieldNames.Count - 1 do
    begin
      lsColumnName := PageName + ' ' + FFieldNames[liIndex] + 'Width';

      aIniFile.WriteInteger('Data Dictionary Builder', lsColumnName, FListView.Columns[liIndex].Width);
    end;
    ...
    begin
      ...
      if not(aDataPipeline.Locate(FFieldNames[0], lsTableName, [pploCaseInsensitive])) then
        begin
          ...
          aDataPipeline.SetFieldValue(FFieldNames[1], ppCapitalize(lsTableAlias));
          aDataPipeline.Post;
...
begin
  ...
  if (aItem = nil) or not(aSelected) or (FListView.SelCount > 1) then
    ...
    begin
      ...
      FcbxDataType.ItemIndex := ppGetEnumStringAsInteger(TypeInfo(TppDataType), aItem.SubItems[2]);
  ...
  while (liIndex < aListItems.Count) and not(FStop) do
    begin
      ...
      {else if table name found, then search for matching field name in fields property of dataset}
        ...
        begin
          lDataSet := FSession.DataSetClass.Create(Self);
          lDataSet.DatabaseName := FDataDictionary.BuilderSettings.DatabaseName;
        ...
        begin
          ...
          if not(aDataPipeline.Locate(FFieldNames[0] + ';' + FFieldNames[1], VarArrayOf([lsTableName, lsFieldName]), [pploCaseInsensitive])) then
            begin
              ...
              aDataPipeline.SetFieldValue(FFieldNames[2], ppCapitalize(lsFieldName));
...
begin
  ...
  inherited CancelMenuClickEvent(Sender);
...
begin
  ...
  while (liIndex1 < aListItems.Count) and not(FStop) do
    begin
      lsTableName1 := aListItems[liIndex1].Caption;
      lsDataName1 := TableNameToSQLString(lsTableName1);

      lsTableName2 := aListItems[liIndex1].SubItems[0];
      lsDataName2 := TableNameToSQLString(lsTableName2);

      if not(FDataDictionary.GetJoinData(lsTableName1, lsTableName2, lsJoinType, lFieldNames1, lFieldNames2, lOperators)) then Continue;

      {search for matching table name in FDataTables, if not found, delete record from data pipeline}
      if (lDataTables.IndexOf(lsTableName1) = -1) or (lDataTables.IndexOf(lsTableName2) = -1) then
        begin
          if (aDataPipeline.Locate(FFieldNames[0] + ';' + FFieldNames[1], VarArrayOf([lsTableName1, lsTableName2]), [pploCaseInsensitive])) then
            begin
              aDataPipeline.Delete;
      ...
      {search for matching field names for first table}
      if not(Result) then
        begin
          lDataSet := FSession.DataSetClass.Create(Self);
          lDataSet.DatabaseName := FDataDictionary.BuilderSettings.DatabaseName;
          lDataSet.DataName := lsDataName1;
          ...
          if OpenDataSet(lDataSet) then
            begin
              ...
              while lbFieldFound and (liIndex2 < lDataSet.FieldCount) and not(FStop) do
                begin
                  lsFieldName := lFieldNames1[liIndex2];
                  ...
                  while not(lbFound) and (liIndex2 < lDataSet.FieldCount) and not(FStop) do
                    begin
                      if (CompareText(lsFieldName, lDataSet.Fields[liIndex3].FieldName) = 0) then
                        lbFound := True
                      else
                        Inc(liIndex3);
                    end;

                  lbFieldFound := lbFound;
      ...
      {search for matching field names for second table}
      if not(Result) then
        begin
          lDataSet := FSession.DataSetClass.Create(Self);
          lDataSet.DatabaseName := FDataDictionary.BuilderSettings.DatabaseName;
          lDataSet.DataName := lsDataName2;
          ...
          if OpenDataSet(lDataSet) then
            begin
              ...
              while lbFieldFound and (liIndex2 < lDataSet.FieldCount) and not(FStop) do
                begin
                  lsFieldName := lFieldNames2[liIndex2];
                  ...
                  if not(lbFieldFound) then
                    begin
                      if (aDataPipeline.Locate(FFieldNames[0] + ';' + FFieldNames[1], VarArrayOf([lsTableName2, lsTableName2]), [pploCaseInsensitive])) then
                        begin
                          aDataPipeline.Delete;
...
begin
  ...
  while (liIndex1 < lDataTables.Count) and not(FStop) do
    begin
      ...
      lDataSet1 := FSession.DataSetClass.Create(Self);
      lDataSet1.DatabaseName := FDataDictionary.BuilderSettings.DatabaseName;
      ...
      while (liIndex2 < lDataTables.Count) and not(FStop) do
        begin
          ...
          lDataSet2 := FSession.DataSetClass.Create(Self);
          lDataSet2.DatabaseName := FDataDictionary.BuilderSettings.DatabaseName;
             ...
             {attempt to find field in table2}
              while not(lbFound) and (liIndex4 < lDataSet2.FieldCount) and not(FStop) do
                begin
                  ...
                  if (lsFieldName1 = lsFieldName2) and
                     (lDataSet1.Fields[liIndex3].DataType = lDataSet2.Fields[liIndex4].DataType) then
                    begin
                      lbFound := True;
