...
uses
  ppClass, ppUtils, ppBands, ppDB, ppCtrls, ppMemo, ppPrintr,
  ppRegion, ppStrtch;
...
begin
  ...
  if aReport is TppCustomReport then
    ...
    begin
      ...
      if lObject.IsClickable and not(lObject = aComponent) then
        begin
          ...
          if FullDescription then
            aList.Add(lObject.Description)
          else
            aList.Add(lObject.Name);
  ...
  for liObject := 0 to TppBand(lBand).ObjectCount - 1 do
    begin
      ...
      if lbAddIt then
        begin
          if FullDescription then
            aList.Add(TppStretchable(lObject).Description)
          else
            aList.Add(lObject.Name);
        end;
...
begin
  ...
  if (lReportDataPipeline <> nil) then
    begin
      ...
      lReportDataPipeline.GetFieldNames(lList1);
      ...
      for liField := 0 to lList1.Count - 1 do
        begin
          ...
          if FullDescription then
            lList1[liField] := lReportDataPipeline.UserName + '.' + lReportDataPipeline.Fields[liField].FieldAlias;
        end;
  ...
  if (lDetailDataPipeline <> nil) then
    begin
      ...
      lDetailDataPipeline.GetFieldNames(lList2);
      ...
      for liField := 0 to lList2.Count - 1 do
        begin
          ...
          if FullDescription then
            lList2[liField] := lDetailDataPipeline.UserName + '.' + lDetailDataPipeline.Fields[liField].FieldAlias;
        end;
...
{$IFDEF WIN32}
  ...
  if (lParent.FormDesigner <> nil) and (lParent.FormDesigner.Designer <> nil) then
    begin
