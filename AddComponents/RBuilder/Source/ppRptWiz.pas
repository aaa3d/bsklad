...
uses
  ...
  ppMemo, ppClasUt, ppWizard, ppRWzDlg, ppVar, ppMetaB, ppMetaRW;
...
type
  ...
  function ppCreateComponentForField(aReport: TppCustomReport; aBand: TppBand; aField: TppField; aFont: TFont): TppComponent;
  function ppCreateFrameForComponent(aReport: TppCustomReport; aComponent: TppComponent): TppShape;
  function ppCreateLabelForField(aReport: TppCustomReport; aBand: TppBand; aField: TppField; aFont: TFont): TppLabel;
  function ppDataTypeToComponentClass(aDataType: TppDataType): TComponentClass;
  procedure ppSetComponentSize(aComponent: TppComponent);
...
begin
  ...
  lComponentClass := ppDataTypeToComponentClass(aField.DataType);
...
end; {procedure, ppCreateComponentForField}
...
end; {procedure, ppSetComponentSize}

{------------------------------------------------------------------------------}
{ ppCreateLabelForField }
...
end; {procedure, ppCreateLabelForField}

{------------------------------------------------------------------------------}
{ ppCreateFrameForComponent }
...
end;  {procedure, ppCreateFrameForComponent}
...
class function TppBlankReportWizard.ClassBitmap: Longint;
begin
...
class function TppReportWizard.ClassBitmap: Longint;
begin
...
begin
  ...
  lbFieldsAvailable := (FForm.lbxGroupFieldsAvailable.Items.Count > 0) and (FGroupFields.Count < 3);
...
begin
  ...
  for liField := 0 to FSelectedFields.Count - 1 do
    begin
      ...
      {get the component class based on the data type}
      lComponentClass := ppDataTypeToComponentClass(lField.DataType);
