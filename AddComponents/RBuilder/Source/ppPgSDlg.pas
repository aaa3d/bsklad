...
uses
  ...
  ExtCtrls, StdCtrls, Printers, Grids,
  ppClass, ppForms, ppTypes, ppUtils, ppBands, ppPrintr;
...
type
  ...
  TppPageSetupDialog = class(TppCustomPageSetupDialog)
    ...
    btnOK: TButton;
    btnCancel: TButton;
    nbkPageSetup: TPageControl;
    tsPaperSize: TTabSheet;
    tsPaperSource: TTabSheet;
    tsLayout: TTabSheet;
    tsMargins: TTabSheet;
    lblPaperSize: TLabel;
    lblWidth: TLabel;
...
begin
  ...
  nbkPageSetup.ActivePage := tsPrinter;
...
begin
  ...
  if (Button = btPrev) then
    FCustomReport.ColumnWidth := FCustomReport.ColumnWidth - FSpinIncrement
  else
    FCustomReport.ColumnWidth := FCustomReport.ColumnWidth + FSpinIncrement;
...
begin
  ...
  if (Button = btPrev) then
    FDetailBand.BottomOffset := FDetailBand.BottomOffset - FSpinIncrement
  else
    FDetailBand.BottomOffset := FDetailBand.BottomOffset + FSpinIncrement;
...
begin
  ...
  if (Button = btPrev) then
    FPrinterSetup.MarginBottom := FPrinterSetup.MarginBottom - FSpinIncrement
  else
    FPrinterSetup.MarginBottom := FPrinterSetup.MarginBottom + FSpinIncrement;
...
begin
  ...
  if (Button = btPrev) then
    FPrinterSetup.MarginTop := FPrinterSetup.MarginTop - FSpinIncrement
  else
    FPrinterSetup.MarginTop := FPrinterSetup.MarginTop + FSpinIncrement;
...
begin
  ...
  if (Button = btPrev) then
    FPrinterSetup.MarginLeft := FPrinterSetup.MarginLeft - FSpinIncrement
  else
    FPrinterSetup.MarginLeft := FPrinterSetup.MarginLeft + FSpinIncrement;
...
begin
  ...
  if (Button = btPrev) then
    FPrinterSetup.MarginRight := FPrinterSetup.MarginRight - FSpinIncrement
  else
    FPrinterSetup.MarginRight := FPrinterSetup.MarginRight + FSpinIncrement;
...
begin
  ...
  nbkPageSetup.Pages[0].Caption := ppLoadStr(25);
  nbkPageSetup.Pages[1].Caption := lblPaperSize.Caption;
  nbkPageSetup.Pages[2].Caption := lblPaperSource.Caption;
  nbkPageSetup.Pages[3].Caption := lblLayout.Caption;
  nbkPageSetup.Pages[4].Caption := lblMargins.Caption;
