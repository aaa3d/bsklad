...
begin
  ...
  aWidth := lCanvas.TextWidth(aText);
  aHeight := lCanvas.TextHeight(aText);
...
begin
  ...
  if (aRow < Matrix.ColumnHeaderCount) then
    begin
      Result := Matrix.CaptionHeight[Matrix.ColumnHeaderCount - 1, aRow];
      ...
      while (liRow < liEndRow) do
        begin
          ...
          if (lbHideLines) and (CrossTab.ValueDefCount > 1) and (liColumn >= Matrix.RowHeaderCount - 2) then
            begin
              {liValueRow := (liRow - Matrix.ColumnHeaderCount);}
...
begin
  ...
  for liRowDef := 0 to CrossTab.RowDefCount - 2 do
    for liRow := liStartRow to liEndRow do
      begin
