...
{$ENDIF}
  ...
  ppRptExp, ppForms, ppTypes, ppUtils, Menus;
...
begin
  FFolderId := Integer(cbxFolders.Items.Objects[cbxFolders.ItemIndex]);

  if (FFolderId = 0) then Exit;

  FFolderId := FReportExplorer.GetParentId(FFolderId);
...
begin
  ...
  FReportExplorer.GetParentFolders(FFolderId, lParentFolders);

  {insert the current folder at the top of the parent folder chain}
  if (FFolderId <> 0) then
    begin
      lFolderInfo := TppFolderInfo.Create;
      lFolderInfo.FolderId := FFolderId;
      lFolderInfo.Name := FReportExplorer.GetFolderName(FFolderId);
      lFolderInfo.ParentId := FReportExplorer.GetParentId(FFolderId);
  ...
  for liIndex := 0 to lFolders.Count - 1 do
    begin
      ...
      if (lFolderInfo.FolderId = FFolderId) then
        liSelected := liIndex;
  ...
  spbUpOneLevel.Enabled := (FFolderId <> 0);
...
begin
  ...
  while (liIndex < FListView.Items.Count) and not(lbFound) do
    begin
      ...
      if ((lItem.ImageIndex + 1) = ItemType) and (CompareText(lItem.Caption, FItemName) = 0) then
        lbFound := True
      else
        Inc(liIndex);
    end;
  ...
  if (DialogType = ppdtOpen) then
    begin
      ...
      lsMessage := Format(lsMessage, [FItemName]);
...
begin
  ...
  lsName := FReportExplorer.GetNewFolderName(FFolderId);

  {create the folder record}
  FReportExplorer.AddFolder(FFolderId, lsName, liFolderId);
...
begin
  ...
  if DialogType = ppdtOpen then
    ...
    begin
      Caption := ppLoadStr(362) + ' ' + FItemName + ' ' +
    ...
    begin
      {draw open folder}
       if (odSelected in State) then
         ImageList1.Draw(cbxFolders.Canvas, Rect.Left + liOffset, Rect.Top, 1)
       else
         ImageList1.Draw(cbxFolders.Canvas, Rect.Left + liOffset, Rect.Top, 0);
    end
  else
    begin
      {draw closed folder}
       if (odSelected in State) then
         ImageList1.Draw(cbxFolders.Canvas, Rect.Left + liOffset, Rect.Top, 3)
       else
         ImageList1.Draw(cbxFolders.Canvas, Rect.Left + liOffset, Rect.Top, 2);
    end;
