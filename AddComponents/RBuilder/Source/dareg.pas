...
uses
  SysUtils, DsgnIntf, Classes, TypInfo,
  ppUtils, ppWizard, ppDB, ppDsgnDB,  ppDBPipe, ppDBBDE, ppTxPipe, ppDBJIT,
  ppDsIntf, ppPropEd,
  daQClass, daDatMod, daQryWiz, daDB, daDBBDE, daDataVw, daDDBldr;
...
begin
  ...
  {child pipelines}
  RegisterNoIcon([TppChildDBPipeline, TppChildBDEPipeline, TppChildTextPipeline, TppChildJITPipeline]);

  {child data access component descendants}
  RegisterNoIcon([TppChildDataSource]);

  {query def components}
  RegisterNoIcon([TdaTable, TdaField, TdaCalculation, TdaTableJoin, TdaCriteria, TdaSQL]);

  {BDE DataAccess Components}
