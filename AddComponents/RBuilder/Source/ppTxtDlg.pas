...
uses
  Windows, ComCtrls,Classes, Graphics, Forms, Controls, Buttons, StdCtrls, Dialogs, SysUtils, ExtCtrls, Mask,
  ppForms, ppTypes, ppUtils, Tabnotbk;
...
begin
  ...
  lReport.AllowPrintToFile := (FSavedCount > 0);
    ...
    begin
      ...
      for liComponent := 0 to liComponents - 1 do
        begin
          ...
          {get the component}
          lComponent := TppCustomText(lSetting.Component);

          {assign the settings}
          if lSetting.Save then
            begin
              lComponent.Save       := lSetting.Save;
              lComponent.SaveOrder  := lSetting.SaveOrder;
              lComponent.SaveLength := lSetting.SaveLength;

              Inc(FSavedCount);
              Inc(liBandSavedCount);
            end
          else
            lComponent.Save := False;
    ...
    begin
      ...
      for liComponent := 0 to liComponents - 1 do
        begin
          ...
          if (lBand.Objects[liComponent] is TppCustomText) then
            begin
              lComponent := TppCustomText(lBand.Objects[liComponent]);

              lSetting := TppSaveSetting.Create(Self);

              lSetting.Band              := lBand;
              lSetting.Component         := lComponent;
              lSetting.ComponentUserName := lComponent.UserName;
              lSetting.Save              := lComponent.Save;
              lSetting.SaveOrder         := lComponent.SaveOrder;
              lSetting.SaveLength        := lComponent.SaveLength;

              {convert SaveOrder to fixed length string}
              if lComponent.Save then
                lsPosition  := Format('%8d',[lComponent.SaveOrder])
              else
                lsPosition := '99999999';
