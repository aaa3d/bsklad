...
uses
  ...
  ppUtils, ppCTCell, ppCTMain, ppDrwCmd, ppPrintr;
...
begin
  ...
  lbAscending := (FCrossTab.ColumnDefs[aColumnDef].SortType = soAscending);
  lbNullsFirst := (FCrossTab.ColumnDefs[aColumnDef].NullOrder = ctnoFirst);
  ...
  while (Result = -1) and (liColumn < liLastDataColumn) do
    begin
      ...
      if (aColumnDef > 0) then
        lCurrentParentLabel := GetValue(FMatrix.Value[liColumn, liRow - 1]);

      {if parent has changed, return column}
      if (aColumnDef > 0) and (lCurrentParentLabel <> lParentLabel) then
        Result := liColumn
      ...
      else if lbAscending and (lLabel < lCurrentLabel) then
        Result := liColumn

      else if not(lbAscending) and (lLabel > lCurrentLabel) then
        Result := liColumn
...
begin
  ...
  if (FCrossTab.ColumnGrandTotals[0].CaptionDef.Visible) then
    Result := Result - 1;
    ...
    begin
      ...
      if not(FMatrix.IsTotalColumn(liColumn)) then
        ...
        begin
          ...
          if (lbAscending and (aParentLabel <= lTotalLabel)) or (not(lbAscending) and (aParentLabel >= lTotalLabel)) then
            begin
              lbSkippingColumns := False;
...
begin
  ...
  while (Result = -1) and (liRow <= FMatrix.RowCount) do
    begin
      ...
      {get current parent label}
      if (aRowDef > 0) then
        lCurrentParentLabel := GetValue(FMatrix.Value[liColumn - 1, liRow]);

      {if parent has changed, return row}
      if (aRowDef > 0) and (lCurrentParentLabel <> lParentLabel) then
        Result := liRow
...
begin
  ...
  FBookmark := FDataPipeline.GetBookmark;
...
begin
  FDataPipeline.GotoBookmark(FBookmark);
  FDataPipeline.FreeBookmark(FBookmark);
