...
begin
  ...
  if (FPrinter = nil) or (FPrinter.PrinterSetup.PrinterNames.Count = 0) then
    ...
    begin
      ...
      cbxPrinters.ItemIndex := cbxPrinters.Items.IndexOf(FPrinter.PrinterSetup.PrinterName);
    end;
  ...
  if (AllowPrintToFile or AllowPrintToArchive) then
    begin
      ...
      SetFileDeviceType(TppProducer(Report).DeviceClass);

      if (TppProducer(Report).DeviceClass = TppArchiveDevice) then
        SaveDialog1.FileName := ArchiveFileName

      else if (TppProducer(Report).DeviceClass.InheritsFrom(TppFileDevice)) then
...
begin
  ...
  else if ((AllowPrintToFile) or (AllowPrintToArchive)) and (cbxPrintToFile.Checked) and (SaveDialog1.FileName = '') then
    {message: 'Please specify file name before continuing.'}
    ShowMessage(ppLoadStr(33))
...
{------------------------------------------------------------------------------}
{ TppPrintDialog.rdbPagesClick }

procedure TppPrintDialog.rdbPagesClick(Sender: TObject);
begin
  rdbAll.Checked := False;
  rdbCurrentPage.Checked := False;
end; {procedure, rdbPagesClick}
...
begin
  ...
  rdbCurrentPage.Caption := ppLoadStr(9);
  rdbPages.Caption := ppLoadStr(20);
