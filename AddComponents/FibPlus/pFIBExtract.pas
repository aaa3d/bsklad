...
type
  ...
  TExtractOption = (CreateDb, Filters, Functions, Domains, AllTables, Index, Foreigns,
    Generators, Views, Checks, Exceptions, Procedures, Triggers, Grants, Describes);
  ...
  protected
    ...
    procedure ListDescribes(DescribeName : String = ''; DescribeFieldName : String = '');  {do not localize}
...
function TpFIBExtract.GetArrayField(FieldName: String): String;
var
  qryArray : TpFIBQuery;
begin
...
begin
  ...
  FExtractOptions := FExtractOptions + [CreateDB, Filters, Functions, Domains, AllTables, Index, Foreigns,
    Generators, Views, Checks, Exceptions, Procedures, Triggers, Grants{,Describes}];
...
function TpFIBExtract.ExtractDDL(Flag: Boolean; TableName: String) : Boolean;
var
	DidConnect : Boolean;
	DidStart : Boolean;
begin
  ...
  begin
    if (CreateDb in ExtractOptions) then ListCreateDb;
    if (Filters in ExtractOptions) then ListFilters;
    if (Functions in ExtractOptions) then ListFunctions;
    if (Domains in ExtractOptions) then ListDomains;
    if (AllTables in ExtractOptions) then ListAllTables(flag);
    if (Index in ExtractOptions) then ListIndex;
    if (Foreigns in ExtractOptions) then ListForeign;
    if (Generators in ExtractOptions) then ListGenerators;
    if (Views in ExtractOptions) then ListViews;
    if (Checks in ExtractOptions) then ListCheck;
    if (Exceptions in ExtractOptions) then ListException;
    if (Procedures in ExtractOptions) then ListProcs;
    if (Triggers in ExtractOptions) then ListTriggers;
    if (Grants in ExtractOptions) then ListGrants;
    if (Describes in ExtractOptions) then ListDescribes;
  end;
...
function TpFIBExtract.ExtractListTable(RelationName, NewName: String;
...
begin
  ...
  try
      ...
      if (not qryTables.FieldByName('RDB$OWNER_NAME').IsNull) and  {do not localize}
        ...
        FMetaData.Add(Format('%s/* Table: %s, Owner: %s */%s',  {do not localize}
          [NEWLINE, RelationName,
           trim(qryTables.FieldByName('RDB$OWNER_NAME').AsString), NEWLINE]));  {do not localize}
      if NewName <> '' then  {do not localize}
        FMetaData.Add(Format('CREATE TABLE %s ', [FormatIdentifier(FDatabase.SQLDialect,NewName)])) {do not localize}
      else
        FMetaData.Add(Format('CREATE TABLE %s ', [FormatIdentifier(FDatabase.SQLDialect,RelationName)]));  {do not localize}
    ...
    begin
      Column := '  ' + FormatIdentifier(FDatabase.SQLDialect, trim(qryTables.FieldByName('RDB$FIELD_NAME').AsString)) + TAB; {do not localize}
    ...
    {  Check first for computed fields, then domains.
      ...
      begin
       ...
       if not qryTables.FieldByName('RDB$COMPUTED_SOURCE').IsNull then {do not localize}
         Column := Column + PrintValidation(trim(qryTables.FieldByName('RDB$COMPUTED_SOURCE').AsString), true); {do not localize}
      end
      else
      begin
        ...
        if not ((Copy(trim(qryTables.FieldByName('RDB$FIELD_NAME1').AsString), 1, 4) = 'RDB$') and {do not localize}
          (trim(qryTables.FieldByName('RDB$FIELD_NAME1').AsString)[5] in ['0'..'9'])) and {do not localize}
          (qryTables.FieldByName('RDB$SYSTEM_FLAG').AsInteger <> 1) then   {do not localize}
        begin
          Column := Column + FormatIdentifier(FDatabase.SQLDialect, trim(qryTables.FieldByName('RDB$FIELD_NAME1').AsString)); {do not localize}
        ...
        begin
          ...
          if not qryTables.FieldByName('RDB$DIMENSIONS').IsNull then   {do not localize}
            Column := column + GetArrayField(trim(qryTables.FieldByName('RDB$FIELD_NAME1').AsString)); {do not localize}
        ...
        begin
          qryConstraints.ParamByName('FIELDNAME').AsString := trim(qryTables.FieldByName('RDB$FIELD_NAME').AsString); {do not localize}
          qryConstraints.ParamByName('RELATIONNAME').AsString := trim(qryTables.FieldByName('RDB$RELATION_NAME').AsString); {do not localize}
          qryConstraints.ExecQuery;

          while not qryConstraints.Eof do
          begin
            if Pos('INTEG', trim(qryConstraints.FieldByName('RDB$CONSTRAINT_NAME').AsString)) <> 1 then {do not localize}
              Column := Column + Format(' CONSTRAINT %s',  {do not localize}
                [ FormatIdentifier( FDatabase.SQLDialect,
                      trim(qryConstraints.FieldByName('RDB$CONSTRAINT_NAME').AsString))]); {do not localize}
            qryConstraints.Next;
          end;
    ...
    begin
      ...
      if Pos('INTEG', trim(qryRelConstraints.FieldByName('RDB$CONSTRAINT_NAME').AsString)) <> 1 then  {do not localize}
        Constraint := Constraint + 'CONSTRAINT ' +   {do not localize}
          FormatIdentifier(FDatabase.SQLDialect,
          trim(qryRelConstraints.FieldByName('RDB$CONSTRAINT_NAME').AsString));  {do not localize}


      if Pos('PRIMARY', trim(qryRelConstraints.FieldByName('RDB$CONSTRAINT_TYPE').AsString)) = 1 then {do not localize}
      begin
        ...
        if Pos('UNIQUE', trim(qryRelConstraints.FieldByName('RDB$CONSTRAINT_TYPE').AsString)) = 1 then  {do not localize}
        begin
...
procedure TpFIBExtract.ExtractListView(ViewName: String);
const
...
begin
  ...
  try
    ...
    begin
      FMetaData.Add('');
      RelationName := FormatIdentifier(FDatabase.SQLDialect,
          trim(qryViews.FieldByName('RDB$RELATION_NAME').AsString));  {do not localize}
      FMetaData.Add(Format('%s/* View: %s, Owner: %s */%s', [  {do not localize}
        RelationName,
        trim(qryViews.FieldByName('RDB$OWNER_NAME').AsString)]));  {do not localize}
      FMetaData.Add('');
...
function TpFIBExtract.GetCharacterSets(CharSetId, Collation: Short;
...
function TpFIBExtract.GetIndexSegments(IndexName: String): String;
const
  IndexNamesSQL =
...
{ Query to get column names }
  ...
  try
    ...
    begin
      ...
      Result := Result + FormatIdentifier(FDatabase.SQLDialect,
        trim(qryColNames.FieldByName('RDB$FIELD_NAME').AsString));  {do not localize}
      qryColNames.Next;
...
procedure TpFIBExtract.ListProcs(ProcedureName : String; AlterOnly : Boolean);
const
...
{  First the dummy procedures
    ...
    begin
      ...
      begin
        ...
        FMetaData.Add(Format(CreateProcedureStr1, [FormatIdentifier(FDatabase.SQLDialect,
           ProcName)]));
...
procedure TpFIBExtract.ListTriggers(ObjectName : String; ExtractType : TExtractType);
const
  ...
  try
    ...
    begin
      ...
      SList.Add(Format('CREATE TRIGGER %s FOR %s %s%s %s POSITION %d',  {do not localize}
	        [FormatIdentifier(FDatabase.SQLDialect, TriggerName),
      ...
      if not qryTriggers.FieldByName('RDB$TRIGGER_SOURCE').IsNull then   {do not localize}
        SList.Text := SList.Text +
              trim(qryTriggers.FieldByName('RDB$TRIGGER_SOURCE').AsString);  {do not localize}
      if IncludeSetTerm then
        SList.Add(' ' + ProcTerm + NEWLINE)
      else
        SList.Add(' ' + Term + NEWLINE);
...
procedure TpFIBExtract.ListCheck(ObjectName : String; ExtractType : TExtractType);
const
    ...
    begin
      ...
      if Pos('INTEG', trim(qryChecks.FieldByName('RDB$CONSTRAINT_NAME').AsString)) <> 1 then    {do not localize}
        SList.Add(Format('%sCONSTRAINT %s ', [TAB,        {do not localize}
          FormatIdentifier(FDatabase.SQLDialect, trim(qryChecks.FieldByName('RDB$CONSTRAINT_NAME').AsString))]));  {do not localize}
...
procedure TpFIBExtract.ListDomains(ObjectName: String; ExtractType : TExtractType);
const
  DomainSQL =
...
var
  ...
  begin
    ...
    for i := Low(ColumnTypes) to High(ColumnTypes) do
      ...
      begin
        ...
        begin
          ...
          begin
            { We are ODS >= 10 and could be any Dialect }
            if (FDatabase.DBSQLDialect >= 3) and
    ...
    if not qryDomains.FieldByName('RDB$VALIDATION_SOURCE').IsNull then   {do not localize}
      if Pos('CHECK', AnsiUpperCase(trim(qryDomains.FieldByName('RDB$VALIDATION_SOURCE').AsString))) = 1 then {do not localize}
        Result := Result + Format('%s%s %s', [NEWLINE, TAB,   {do not localize}
...
procedure TpFIBExtract.ListException(ExceptionName : String = '');
const
  ExceptionSQL =
  ...
  try
    ...
    begin
      ...
      FMetaData.Add(Format('CREATE EXCEPTION %s %s%s',  {do not localize}
        [FormatIdentifier(FDatabase.SQLDialect, trim(qryException.FieldByName('RDB$EXCEPTION_NAME').AsString)), {do not localize}
...
procedure TpFIBExtract.ListFilters(FilterName : String = '');
const
  FiltersSQL =
...
procedure TpFIBExtract.ListForeign(ObjectName : String; ExtractType : TExtractType);
const
...
procedure TpFIBExtract.ListFunctions(FunctionName : String = '');
const
  FunctionSQL =
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          PrecisionKnown := false;
          if (FDatabase.ODSMajorVersion >= ODS_VERSION10) and
...
procedure TpFIBExtract.ListGenerators(GeneratorName : String = '');   {do not localize}
const
  GeneratorSQL =
  ...
  try
    ...
    begin
      ...
      FMetaData.Add(Format('CREATE GENERATOR %s%s',   {do not localize}
        [FormatIdentifier(FDatabase.SQLDialect, GenName),
         Term]));
      qryGenerator.Next;
    end;
  finally
...
procedure TpFIBExtract.ListIndex(ObjectName : String; ExtractType : TExtractType);
const
  IndexSQL =
  ...
  try
    ...
    begin
      ...
      Line := Format('CREATE%s%s INDEX %s ON %s(', [Unique, IdxType, {do not localize}
        FormatIdentifier(FDataBase.SQLDialect,
            trim(qryIndex.FieldByName('RDB$INDEX_NAME').AsString)),   {do not localize}
        FormatIdentifier(FDataBase.SQLDialect,
            trim(qryIndex.FieldByName('RDB$RELATION_NAME').AsString))]);   {do not localize}
...
procedure TpFIBExtract.ListViews(ViewName : String);
const
  ViewSQL =
...
procedure TpFIBExtract.ListDescribes(DescribeName : String = ''; DescribeFieldName : String = '');
const
  DescribeSQL =
    'select * from RDB$RELATIONS ' +  {do not localize}
    'WHERE RDB$DESCRIPTION <> ''' +  {do not localize}
  ...
  DescribeFieldNameSQL =
    'select * from RDB$RELATION_FIELDS ' +  {do not localize}
    'WHERE RDB$RELATION_NAME = :DescribeName ' + {do not localize}
...
begin
  ...
  try
    ...
    begin
      ...
      if DescribeFieldName = '' then {do not localize}
        //examples DESCRIBE TABLE COMPONENTS 'компоненты';
        FMetaData.Add(Format('DESCRIBE TABLE %s %s%s',  {do not localize}
          [FormatIdentifier(FDatabase.SQLDialect, trim(qryDescribe.FieldByName('RDB$RELATION_NAME').AsString)), {do not localize}
          QuotedStr(trim(qryDescribe.FieldByName('RDB$DESCRIPTION').AsString)), Term]))  {do not localize}
      else
        //examples DESCRIBE FIELD NAME TABLE COMPONENTS 'поле имени';
        FMetaData.Add(Format('DESCRIBE FIELD %s TABLE %s %s%s',  {do not localize}
          [FormatIdentifier(FDatabase.SQLDialect, trim(qryDescribe.FieldByName('RDB$FIELD_NAME').AsString)), {do not localize}
          FormatIdentifier(FDatabase.SQLDialect, trim(qryDescribe.FieldByName('RDB$RELATION_NAME').AsString)), {do not localize}
          QuotedStr(trim(qryDescribe.FieldByName('RDB$DESCRIPTION').AsString)), Term]));  {do not localize}
...
function TpFIBExtract.PrintValidation(ToValidate: String;
  flag: Boolean): String;
var
  IsSQL : Boolean;
begin
...
procedure TpFIBExtract.ExtractObject(ObjectType : TExtractObjectTypes;
...
function TpFIBExtract.GetFieldType(FieldType, FieldSubType, FieldScale,
...
procedure TpFIBExtract.ShowGrants(MetaObject, Terminator: String);
const
    ...
    begin
      ...
      begin
        ...
        FMetaData.Add(Format('GRANT %s%s ON %s TO %s%s%s', [PrivString, {do not localize}
          ColString, FormatIdentifier(FDatabase.SQLDialect, MetaObject),
     ...
     { Part two is for stored procedures only }
      ...
      begin
        ...
        FMetaData.Add(Format('GRANT EXECUTE ON PROCEDURE %s TO %s%s%s',  {do not localize}
          [FormatIdentifier(FDatabase.SQLDialect, MetaObject), UserString,
    ...
    begin
      ...
      begin
        ...
        FMetaData.Add(Format('GRANT %s TO %s%s%s',      {do not localize}
          [FormatIdentifier(FDatabase.SQLDialect, trim(qryOwnerPriv.FieldByName('RDB$RELATION_NAME').AsString)),  {do not localize}
...
procedure TpFIBExtract.ShowGrantRoles(Terminator: String);
const
  RoleSQL =
  ...
  try
    ...
    begin
      ...
      FMetaData.Add(Format('GRANT %s TO %s%s%s%s',    {do not localize}
        [ FormatIdentifier(FDatabase.SQLDialect, trim(qryRole.FieldByName('RDB$RELATION_NAME').AsString)),  {do not localize}
...
procedure TpFIBExtract.GetProcedureArgs(Proc: String);
const
...
var
  ...
  begin
    Result := Format('  %s ', [FormatIdentifier(FDatabase.SQLDialect, trim(qryHeader.FieldByName('RDB$PARAMETER_NAME').AsString))]); {do not localize}
    for i := Low(ColumnTypes) to High(ColumnTypes) do
...
procedure TpFIBExtract.Notification(AComponent: TComponent;
...
procedure TpFIBExtract.ListData(ObjectName: String);
const
...
begin
  ...
  try
    qrySelect.SQL.Text := Format(SelectSQL,
      [FormatIdentifier(FDatabase.SQLDialect, ObjectName)]);
    qrySelect.ExecQuery;
//?
(*    Fields := '';
    ...
    begin
      Line := 'INSERT INTO ' + FormatIdentifier(FDatabase.SQLDialect, ObjectName) + ' (';  {do not localize}
//insert
      for i := 0 to qrySelect.Current.Count - 1 do
        if (qrySelect.Fields[i].SQLType <> SQL_ARRAY) and
           (qrySelect.Fields[i].SQLType <> SQL_BLOB) then
...
procedure TpFIBExtract.ListRoles(ObjectName: String);
const
  RolesSQL =
  ...
  try
    ...
    begin
      ...
      try
        while not qryRoles.Eof do
        begin
          RoleName := FormatIdentifier(FDatabase.SQLDialect,
              trim(qryRoles.FieldByName('rdb$Role_Name').AsString));  {do not localize}
