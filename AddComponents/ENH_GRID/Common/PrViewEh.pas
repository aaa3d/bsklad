...
//    function Previewer: TPreviewBox;
  protected
    ...
    procedure DrawPage(Sender: TObject; Canvas: TCanvas;
      PageNumber: Integer);
...
begin
  ...
  begin
    Parent := TPreviewBox(Self.Parent);
    if Parent.ViewMode = vmFullPage then begin
...
begin
  ...
  if (Operation = opRemove) and (AComponent = PrinterSetupOwner) then
...
begin
  ...
  if Printer.Printers.Count > 0 then
    FMetafileCanvas := TMetafileCanvas.Create(FMetafileList[FMetafileList.Count-1],Printer.Handle)
  else
    FMetafileCanvas := TMetafileCanvas.Create(FMetafileList[FMetafileList.Count-1],Application.MainForm.Canvas.Handle);
  ...
  if Assigned(Previewer.OnPrinterPreviewChanged) then Previewer.OnPrinterPreviewChanged(Self);
end;
...
begin
  ...
  if FMetafileList.Count = 2 then Previewer.UpdatePageSetup; //UpdatePreview;
  if Printer.Printers.Count > 0 then
    FMetafileCanvas := TMetafileCanvas.Create(FMetafileList[FMetafileList.Count-1],Printer.Handle)
  else
    FMetafileCanvas := TMetafileCanvas.Create(FMetafileList[FMetafileList.Count-1],Application.MainForm.Canvas.Handle);
...
begin
  if Printer.Printers.Count > 0 then
    ...
    Result := DefaultPrinterPageHeight;
end;
...
begin
  if Printer.Printers.Count > 0 then
    ...
    Result := DefaultPrinterPageWidth;
end;
