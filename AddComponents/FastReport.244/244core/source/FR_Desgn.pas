...
{  sl := TStringList.Create;
  frGetComponents(Self, TfrSpeedButton, sl, nil);
  b := TBitmap.Create;
  b.Width := 16;
  b.Height := 16;
  ...
  begin
    sb := frFindComponent(Self, sl[i]) as TfrSpeedButton;
    if (sb = nil) or (Pos('.', sl[i]) <> 0) then continue;
    sb.DrawGlyph(b.Canvas, 0, 0, True);
    b.SaveToFile('d:\bmps\e' + IntToStr(i) + '.bmp');
    sb.DrawGlyph(b.Canvas, 0, 0, False);
    b.SaveToFile('d:\bmps\d' + IntToStr(i) + '.bmp');
  end;
