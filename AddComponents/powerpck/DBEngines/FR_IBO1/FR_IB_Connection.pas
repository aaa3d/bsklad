...
uses
  FR_Utils, FR_DBUtils, FR_Const, FR_LEdit, FR_DBLookupCtl, FR_IB_Query,
  Variants;
...
procedure TfrIB_Connection.DBNameEditor(Sender: TObject);
var
  SaveConnected: Bool;
begin
  SaveConnected := FConnection.Connected;
  ...
  begin
    ...
    Filter := 'Databases (*.gdb)|*.gdb|All files (*.*)|*.*';
    if Execute then
