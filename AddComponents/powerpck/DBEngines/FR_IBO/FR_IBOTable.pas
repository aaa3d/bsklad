...
uses
  ...
  Controls, Forms, Menus, Dialogs, DB, IBODataset, FR_DBSet;

type
  TfrIBODataset = class(TfrNonVisualControl)
  protected
  ...
  TfrIBOTable = class(TfrIBODataSet)
  private
    FTable: TIBOTable;
  ...
  public
    ...
    procedure DefineProperties; override;
    procedure Loaded; override;
    property Table: TIBOTable read FTable;
  end;


implementation

uses FR_DBUtils, FR_Utils, FR_Const, FR_LEdit, FR_DBFldEditor, FR_IBOMd;


{ TfrIBODataSet }

constructor TfrIBODataSet.Create;
begin
  inherited Create;
...
destructor TfrIBODataSet.Destroy;
begin
  FDBDataset.Free;
  FDataSource.Free;
  FDataSet.Free;
...
procedure TfrIBODataSet.DefineProperties;
  ...
  begin
    Result := '';
    sl := TStringList.Create;
    frGetComponents(frDialogForm, TIBODatabase, sl, nil);
    sl.Sort;
    for i := 0 to sl.Count - 1 do
      Result := Result + sl[i] + ';';
    sl.Free;
...
begin
  ...
  AddEnumProperty('DatabaseName', GetDataBases, [Null]);
...
procedure TfrIBODataSet.SetPropValue(Index: String; Value: Variant);
begin
...
function TfrIBODataSet.GetPropValue(Index: String): Variant;
begin
  Index := AnsiUpperCase(Index);
...
function TfrIBODataSet.DoMethod(MethodName: String; Par1, Par2, Par3: Variant): Variant;
begin
...
procedure TfrIBODataSet.ReadFields(Stream: TStream);
var
  i: Integer;
  n: Word;
  s: String;
...
// Old version of IBOComponents stores fieldlist wrongfully
...
procedure TfrIBODataSet.WriteFields(Stream: TStream);
var
  i: Integer;
  s: String;
  SaveActive: Boolean;
...
procedure TfrIBODataSet.Loaded;
var
  i: Integer;
  s: String;
  ds: TDataSet;
...
procedure TfrIBODataSet.ShowEditor;
begin
  FieldsEditor(nil);
end;

procedure TfrIBODataSet.FieldsEditor(Sender: TObject);
var
  SaveActive: Boolean;
begin
  SaveActive := FDataSet.Active;
...
constructor TfrIBOTable.Create;
begin
  inherited Create;
  FTable := TIBOTable.Create(frDialogForm);
  FDataSet := FTable;
  FDataSource.DataSet := FDataSet;

  Component := FTable;
  BaseName := 'Table';
  Bmp.LoadFromResourceName(hInstance, 'FR_IBOTABLE');
end;

procedure TfrIBOTable.DefineProperties;
...
procedure TfrIBOTable.SetPropValue(Index: String; Value: Variant);
var
  d: TDataset;
begin
  inherited SetPropValue(Index, Value);
...
function TfrIBOTable.GetPropValue(Index: String): Variant;
begin
  Index := AnsiUpperCase(Index);
  Result := inherited GetPropValue(Index);
  if Result <> Null then Exit;
...
procedure TfrIBOTable.LoadFromStream(Stream: TStream);
begin
  FFixupList.Clear;
...
procedure TfrIBOTable.SaveToStream(Stream: TStream);
begin
  LVersion := 1;
...
procedure TfrIBOTable.Loaded;
begin
  Prop['MasterSource'] := FFixupList['MasterSource'];
  inherited Loaded;
end;

procedure TfrIBOTable.JoinEditor(Sender: TObject);
begin
  with TfrIBOFieldsLinkForm.Create(nil) do
  begin
...
initialization
  Bmp := TBitmap.Create;
  Bmp.LoadFromResourceName(hInstance, 'FR_IBOTABLECONTROL');
  frRegisterControl(TfrIBOTable, Bmp, IntToStr(SInsertTable));

finalization
  frUnRegisterObject(TfrIBOTable);
  Bmp.Free;
