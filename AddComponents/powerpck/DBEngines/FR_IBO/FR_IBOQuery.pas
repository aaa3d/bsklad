...
uses
  ...
  Menus, Dialogs, FR_Class, FR_Pars, DB, IBODataset, FR_IBOTable, FR_DBUtils;

type
  TfrIBOQuery = class(TfrIBODataSet)
  private
    FQuery: TIBOQuery;
  ...
  public
    ...
    property Query: TIBOQuery read FQuery;
...
uses
  FR_Utils, FR_Const, FR_DBSQLEdit, FR_IBOQueryParam
{$IFDEF QBUILDER}
, FR_IBOQB, QBuilder
{$ENDIF};
...
constructor TfrIBOQuery.Create;
begin
  inherited Create;
  FQuery := TIBOQuery.Create(frDialogForm);
  ...
  Component := FQuery;
  BaseName := 'Query';
  Bmp.LoadFromResourceName(hInstance, 'FR_IBOQUERY');
end;

destructor TfrIBOQuery.Destroy;
begin
  FParams.Free;
  inherited Destroy;
end;

procedure TfrIBOQuery.DefineProperties;
...
procedure TfrIBOQuery.SetPropValue(Index: String; Value: Variant);
var
  d: TDataset;
begin
...
function TfrIBOQuery.GetPropValue(Index: String): Variant;
begin
  Index := AnsiUpperCase(Index);
...
function TfrIBOQuery.DoMethod(MethodName: String; Par1, Par2, Par3: Variant): Variant;
begin
...
procedure TfrIBOQuery.LoadFromStream(Stream: TStream);
var
  s: String;
begin
  FFixupList.Clear;
...
procedure TfrIBOQuery.SaveToStream(Stream: TStream);
begin
  LVersion := 1;
...
procedure TfrIBOQuery.Loaded;
begin
  Prop['DataSource'] := FFixupList['DataSource'];
  inherited Loaded;
end;

procedure TfrIBOQuery.SQLEditor(Sender: TObject);
begin
...
{$IFDEF QBUILDER}
    QBEngine := TfrQBIBOEngine.Create(nil);
    TfrQBIBOEngine(QBEngine).Query := FQuery;
...
procedure TfrIBOQuery.ParamsEditor(Sender: TObject);
var
  Params: TParams;
...
function TfrIBOQuery.GetParamKind(Index: Integer): TfrParamKind;
begin
  Result := pkValue;
...
procedure TfrIBOQuery.SetParamKind(Index: Integer; Value: TfrParamKind);
begin
...
function TfrIBOQuery.GetParamText(Index: Integer): String;
begin
  Result := '';
...
procedure TfrIBOQuery.SetParamText(Index: Integer; Value: String);
begin
  if ParamKind[Index] = pkValue then
    FParams[FQuery.Params[Index].Name] := Value;
end;

procedure TfrIBOQuery.ReadParams(Stream: TStream);
var
  i: Integer;
  w, n: Word;
begin
...
procedure TfrIBOQuery.WriteParams(Stream: TStream);
var
  i: Integer;
  w: Word;
begin
...
procedure TfrIBOQuery.BeforeOpenQuery(DataSet: TDataSet);
var
  i: Integer;
  SaveView: TfrView;
  SavePage: TfrPage;
...
initialization
  Bmp := TBitmap.Create;
  Bmp.LoadFromResourceName(hInstance, 'FR_IBOQUERYCONTROL');
  frRegisterControl(TfrIBOQuery, Bmp, IntToStr(SInsertQuery));

finalization
  frUnRegisterObject(TfrIBOQuery);
  Bmp.Free;
