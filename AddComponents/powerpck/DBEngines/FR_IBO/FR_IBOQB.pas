...
uses
  ...
  StdCtrls, ExtCtrls, DB, IBODataset, QBuilder;


type
  TfrQBIBOEngine = class(TOQBEngine)
  private
    FResultQuery: TIBOQuery;
  public
  ...
  published
    property Query: TIBOQuery read FResultQuery write FResultQuery;
  end;
...
constructor TfrQBIBOEngine.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FResultQuery := nil;
end;

destructor TfrQBIBOEngine.Destroy;
begin
  CloseResultQuery;
  inherited Destroy;
end;

procedure TfrQBIBOEngine.UpdateTableList;
begin
  ReadTableList;
end;

function TfrQBIBOEngine.SelectDatabase: Boolean;
begin
  Result := False;
end;

procedure TfrQBIBOEngine.ReadTableList;
begin
  Session.GetTableNames(DatabaseName, '', True, ShowSystemTables, TableList);
end;

procedure TfrQBIBOEngine.ReadFieldList(ATableName: String);
var
  i: Integer;
  Temp: TIBOTable;
begin
  Temp := nil;
  try
    Temp := TIBOTable.Create(frDialogForm);
...
procedure TfrQBIBOEngine.ClearQuerySQL;
begin
  FResultQuery.SQL.Clear;
end;

procedure TfrQBIBOEngine.SetQuerySQL(Value: String);
begin
  FResultQuery.SQL.Text := Value;
end;

function TfrQBIBOEngine.ResultQuery: TDataSet;
begin
  Result := FResultQuery;
end;

procedure TfrQBIBOEngine.OpenResultQuery;
begin
...
procedure TfrQBIBOEngine.CloseResultQuery;
begin
  FResultQuery.Active := False;
end;

procedure TfrQBIBOEngine.SaveResultQueryData;
begin
