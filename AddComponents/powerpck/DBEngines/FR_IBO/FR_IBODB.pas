...
uses
  ...
  Controls, Forms, Menus, Dialogs, DB, IBODataset;

type
  TfrIBOComponents = class(TComponent) // fake component
  end;

  TfrIBODatabase = class(TfrNonVisualControl)
  private
...
uses FR_Utils, FR_Const, FR_LEdit, FR_DBLookupCtl, FR_IBOTable, FR_IBOQuery;
...
constructor TfrIBODatabase.Create;
begin
  inherited Create;
  FDatabase := TIBODatabase.Create(frDialogForm);
  Component := FDatabase;
  BaseName := 'Database';
  Bmp.LoadFromResourceName(hInstance, 'FR_IBODB');
  Flags := Flags or flDontUndo;
end;

destructor TfrIBODatabase.Destroy;
begin
  FDatabase.Free;
  inherited Destroy;
end;

procedure TfrIBODatabase.DefineProperties;
...
procedure TfrIBODatabase.SetPropValue(Index: String; Value: Variant);
begin
...
function TfrIBODatabase.GetPropValue(Index: String): Variant;
begin
  Index := AnsiUpperCase(Index);
...
function TfrIBODataBase.DoMethod(MethodName: String; Par1, Par2, Par3: Variant): Variant;
begin
...
procedure TfrIBODatabase.LoadFromStream(Stream: TStream);
var
  s: String;
begin
...
procedure TfrIBODatabase.SaveToStream(Stream: TStream);
begin
...
procedure TfrIBODatabase.LinesEditor(Sender: TObject);
var
  sl: TStringList;
...
initialization
  Bmp := TBitmap.Create;
  Bmp.LoadFromResourceName(hInstance, 'FR_IBODBCONTROL');
  frRegisterControl(TfrIBODatabase, Bmp, IntToStr(SInsertDB));

finalization
  frUnRegisterObject(TfrIBODatabase);
  Bmp.Free;
