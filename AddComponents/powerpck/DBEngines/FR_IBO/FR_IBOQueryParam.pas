...
uses
  ...
  StdCtrls, FR_Ctrls, ExtCtrls, DB, IBODataset, FR_IBOQuery;
...
procedure TfrIBOParamsForm.TypeCBChange(Sender: TObject);
begin
...
procedure TfrIBOParamsForm.ValueRBClick(Sender: TObject);
begin
  if FBusy then Exit;
  ValueE.Enabled := True;
...
procedure TfrIBOParamsForm.AssignRBClick(Sender: TObject);
begin
  if FBusy then Exit;
  ValueE.Text := '';
  ValueE.Enabled := False;
