...
uses
  ...
  StdCtrls, FR_Ctrls, ExtCtrls, DB, CTQuery, FR_CTLQuery;

type
  TfrCTLParamsForm = class(TForm)
    GroupBox1: TGroupBox;
    Label2: TLabel;
    ParamsLB: TListBox;
    TypeCB: TComboBox;
...
function TfrCTLParamsForm.CurParam: Integer;
var
  i: Integer;
  s: String;
begin
...
procedure TfrCTLParamsForm.FormShow(Sender: TObject);
var
  i: Integer;
begin
...
procedure TfrCTLParamsForm.FormHide(Sender: TObject);
begin
  if ValueE.Enabled then ValueEExit(nil);
end;

procedure TfrCTLParamsForm.ParamsLBClick(Sender: TObject);
var
  i: Integer;
begin
  TypeCB.ItemIndex := -1;
...
procedure TfrCTLParamsForm.ValueEExit(Sender: TObject);
begin
  QueryComp.ParamText[CurParam] := ValueE.Text;
end;

procedure TfrCTLParamsForm.TypeCBChange(Sender: TObject);
begin
...
procedure TfrCTLParamsForm.ValueRBClick(Sender: TObject);
begin
  if FBusy then Exit;
  ValueE.Enabled := True;
...
procedure TfrCTLParamsForm.AssignRBClick(Sender: TObject);
begin
  if FBusy then Exit;
  ValueE.Text := '';
  ValueE.Enabled := False;
...
procedure TfrCTLParamsForm.Localize;
begin
...
procedure TfrCTLParamsForm.FormCreate(Sender: TObject);
begin
  Localize;
end;

procedure TfrCTLParamsForm.VarSBClick(Sender: TObject);
begin
