...
uses
  ...
  StdCtrls, FR_Ctrls, ExtCtrls, DB, DBTables, FR_OCIQuery,
  NCOci, NCOciWrapper, NCOciDB;

type
  TfrOCIParamsForm = class(TForm)
    GroupBox1: TGroupBox;
    Label2: TLabel;
    ParamsLB: TListBox;
    TypeCB: TComboBox;
...
function TfrOCIParamsForm.CurParam: Integer;
var
  i: Integer;
  s: String;
begin
...
procedure TfrOCIParamsForm.FormShow(Sender: TObject);
var
  i: Integer;
begin
...
procedure TfrOCIParamsForm.FormHide(Sender: TObject);
begin
  if ValueE.Enabled then ValueEExit(nil);
end;

procedure TfrOCIParamsForm.ParamsLBClick(Sender: TObject);
var
  i: Integer;
begin
  TypeCB.ItemIndex := -1;
...
procedure TfrOCIParamsForm.ValueEExit(Sender: TObject);
begin
  QueryComp.ParamText[CurParam] := ValueE.Text;
end;

procedure TfrOCIParamsForm.TypeCBChange(Sender: TObject);
begin
...
procedure TfrOCIParamsForm.ValueRBClick(Sender: TObject);
begin
  if FBusy then Exit;
  ValueE.Enabled := True;
...
procedure TfrOCIParamsForm.AssignRBClick(Sender: TObject);
begin
  if FBusy then Exit;
  ValueE.Text := '';
  ValueE.Enabled := False;
...
procedure TfrOCIParamsForm.Localize;
begin
...
procedure TfrOCIParamsForm.FormCreate(Sender: TObject);
begin
  Localize;
end;

procedure TfrOCIParamsForm.VarSBClick(Sender: TObject);
begin
