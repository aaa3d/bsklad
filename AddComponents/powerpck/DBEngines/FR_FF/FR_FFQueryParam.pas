...
uses
  ...
  StdCtrls, FR_Ctrls, ExtCtrls, DB, FFDB, FR_FFQuery;

type
  TfrFFParamsForm = class(TForm)
    GroupBox1: TGroupBox;
    Label2: TLabel;
    ParamsLB: TListBox;
    TypeCB: TComboBox;
...
function TfrFFParamsForm.CurParam: Integer;
var
  i: Integer;
  s: String;
begin
...
procedure TfrFFParamsForm.FormShow(Sender: TObject);
var
  i: Integer;
begin
...
procedure TfrFFParamsForm.FormHide(Sender: TObject);
begin
  if ValueE.Enabled then ValueEExit(nil);
end;

procedure TfrFFParamsForm.ParamsLBClick(Sender: TObject);
var
  i: Integer;
begin
  TypeCB.ItemIndex := -1;
...
procedure TfrFFParamsForm.ValueEExit(Sender: TObject);
begin
  QueryComp.ParamText[CurParam] := ValueE.Text;
end;

procedure TfrFFParamsForm.TypeCBChange(Sender: TObject);
begin
...
procedure TfrFFParamsForm.ValueRBClick(Sender: TObject);
begin
  if FBusy then Exit;
  ValueE.Enabled := True;
...
procedure TfrFFParamsForm.AssignRBClick(Sender: TObject);
begin
  if FBusy then Exit;
  ValueE.Text := '';
  ValueE.Enabled := False;
...
procedure TfrFFParamsForm.Localize;
begin
...
procedure TfrFFParamsForm.FormCreate(Sender: TObject);
begin
  Localize;
end;

procedure TfrFFParamsForm.VarSBClick(Sender: TObject);
begin
