...
uses
  ...
  Menus, Dialogs, FR_Class, FR_Pars, DB, FFDB, FR_FFTable, FR_DBUtils;

type
  TfrFFQuery = class(TfrFFDataSet)
  private
    FQuery: TFFQuery;
...
uses
  FR_Utils, FR_Const, FR_DBSQLEdit, FR_FFQueryParam
{$IFDEF Delphi6}
, Variants
{$ENDIF}
{$IFDEF QBUILDER}
...
constructor TfrFFQuery.Create;
begin
  inherited Create;
  FQuery := TFFQuery.Create(frDialogForm);
  ...
  Component := FQuery;
  BaseName := 'Query';
  Bmp.LoadFromResourceName(hInstance, 'FR_FFQUERY');
end;

destructor TfrFFQuery.Destroy;
begin
  FParams.Free;
  inherited Destroy;
end;

procedure TfrFFQuery.DefineProperties;
...
procedure TfrFFQuery.SetPropValue(Index: String; Value: Variant);
var
  d: TDataset;
begin
...
function TfrFFQuery.GetPropValue(Index: String): Variant;
begin
  Index := AnsiUpperCase(Index);
  ...
  if Index = 'DATASOURCE' then
    ...
    Result := FQuery.Timeout;
end;

function TfrFFQuery.DoMethod(MethodName: String; Par1, Par2, Par3: Variant): Variant;
begin
...
procedure TfrFFQuery.LoadFromStream(Stream: TStream);
var
  s: String;
begin
  FFixupList.Clear;
  ...
  ReadFields(Stream);
  ReadParams(Stream);
  FQuery.SessionName := frReadString(Stream);  
  FQuery.Timeout := frReadInteger(Stream);
  try
...
procedure TfrFFQuery.SaveToStream(Stream: TStream);
begin
  LVersion := 1;
  ...
  WriteFields(Stream);
  WriteParams(Stream);
  frWriteString(Stream, FQuery.SessionName);                 
  frWriteInteger(Stream, FQuery.Timeout);
end;

procedure TfrFFQuery.Loaded;
begin
  Prop['DataSource'] := FFixupList['DataSource'];
  inherited Loaded;
end;

procedure TfrFFQuery.SQLEditor(Sender: TObject);
begin
...
{$IFDEF QBUILDER}
    QBEngine := TfrQBFFEngine.Create(nil);
    TfrQBFFEngine(QBEngine).Query := FQuery;
...
procedure TfrFFQuery.ParamsEditor(Sender: TObject);
var
  Params: TParams;
...
begin
  ...
  begin
    ...
    with TfrFFParamsForm.Create(nil) do
    begin
      QueryComp := Self;
      Query := FQuery;
...
function TfrFFQuery.GetParamKind(Index: Integer): TfrParamKind;
begin
  Result := pkValue;
...
procedure TfrFFQuery.SetParamKind(Index: Integer; Value: TfrParamKind);
begin
...
function TfrFFQuery.GetParamText(Index: Integer): String;
begin
  Result := '';
...
procedure TfrFFQuery.SetParamText(Index: Integer; Value: String);
begin
  if ParamKind[Index] = pkValue then
    FParams[FQuery.Params[Index].Name] := Value;
end;

procedure TfrFFQuery.ReadParams(Stream: TStream);
var
  i: Integer;
  w, n: Word;
begin
...
procedure TfrFFQuery.WriteParams(Stream: TStream);
var
  i: Integer;
  w: Word;
begin
...
procedure TfrFFQuery.BeforeOpenQuery(DataSet: TDataSet);
var
  i: Integer;
  SaveView: TfrView;
  SavePage: TfrPage;
...
initialization
  Bmp := TBitmap.Create;
  Bmp.LoadFromResourceName(hInstance, 'FR_FFQUERYCONTROL');
  frRegisterControl(TfrFFQuery, Bmp, IntToStr(SInsertQuery));

finalization
  frUnRegisterObject(TfrFFQuery);
  Bmp.Free;
