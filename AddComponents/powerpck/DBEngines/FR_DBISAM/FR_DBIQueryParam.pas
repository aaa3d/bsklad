...
uses
  ...
  StdCtrls, FR_Ctrls, ExtCtrls, DB, DBISAMtb, FR_DBIQuery;

type
  TfrDBIParamsForm = class(TForm)
    GroupBox1: TGroupBox;
    Label2: TLabel;
    ParamsLB: TListBox;
    TypeCB: TComboBox;
...
function TfrDBIParamsForm.CurParam: Integer;
var
  i: Integer;
  s: String;
begin
...
procedure TfrDBIParamsForm.FormShow(Sender: TObject);
var
  i: Integer;
begin
...
procedure TfrDBIParamsForm.FormHide(Sender: TObject);
begin
  if ValueE.Enabled then ValueEExit(nil);
end;

procedure TfrDBIParamsForm.ParamsLBClick(Sender: TObject);
var
  i: Integer;
begin
  TypeCB.ItemIndex := -1;
...
procedure TfrDBIParamsForm.ValueEExit(Sender: TObject);
begin
  QueryComp.ParamText[CurParam] := ValueE.Text;
end;

procedure TfrDBIParamsForm.TypeCBChange(Sender: TObject);
begin
...
procedure TfrDBIParamsForm.ValueRBClick(Sender: TObject);
begin
  if FBusy then Exit;
  ValueE.Enabled := True;
...
procedure TfrDBIParamsForm.AssignRBClick(Sender: TObject);
begin
  if FBusy then Exit;
  ValueE.Text := '';
  ValueE.Enabled := False;
...
procedure TfrDBIParamsForm.Localize;
begin
...
procedure TfrDBIParamsForm.FormCreate(Sender: TObject);
begin
  Localize;
end;

procedure TfrDBIParamsForm.VarSBClick(Sender: TObject);
begin
