...
uses
  ...
  Menus, Dialogs, FR_Class, FR_Pars, DB, DBISAMtb, FR_DBITable, FR_DBUtils;

type
  TfrDBIQuery = class(TfrDBIDataSet)
  private
    FQuery: TDBISAMQuery;
  ...
  public
    ...
    property Query: TDBISAMQuery read FQuery;
...
{$IFDEF Delphi6}
  Variants,
{$ENDIF}
  FR_Utils, FR_Const, FR_DBSQLEdit, FR_DBIQueryParam
{$IFDEF QBUILDER}
, FR_DBIQB, QBuilder
{$ENDIF};
...
constructor TfrDBIQuery.Create;
begin
  inherited Create;
  FQuery := TDBISAMQuery.Create(frDialogForm);
  ...
  Component := FQuery;
  BaseName := 'Query';
  Bmp.LoadFromResourceName(hInstance, 'FR_DBIQUERY');
end;

destructor TfrDBIQuery.Destroy;
begin
  FParams.Free;
  inherited Destroy;
end;

procedure TfrDBIQuery.DefineProperties;
...
procedure TfrDBIQuery.SetPropValue(Index: String; Value: Variant);
var
  d: TDataset;
begin
...
function TfrDBIQuery.GetPropValue(Index: String): Variant;
begin
  Index := AnsiUpperCase(Index);
...
function TfrDBIQuery.DoMethod(MethodName: String; Par1, Par2, Par3: Variant): Variant;
begin
...
procedure TfrDBIQuery.LoadFromStream(Stream: TStream);
var
  s: String;
begin
  FFixupList.Clear;
...
procedure TfrDBIQuery.SaveToStream(Stream: TStream);
begin
  LVersion := 1;
...
procedure TfrDBIQuery.Loaded;
begin
  Prop['DataSource'] := FFixupList['DataSource'];
  inherited Loaded;
end;

procedure TfrDBIQuery.SQLEditor(Sender: TObject);
begin
...
{$IFDEF QBUILDER}
    QBEngine := TfrQBDBIEngine.Create(nil);
    TfrQBDBIEngine(QBEngine).Query := FQuery;
...
procedure TfrDBIQuery.ParamsEditor(Sender: TObject);
var
  Params: TParams;
  ParamValues: TfrVariables; I: integer;
begin
...
//    Params.Assign(FQuery.Params);
    ...
    for i:=0 to FQuery.ParamCount-1 do
     begin
      Params.Add;
      Params.Items[i].Name := FQuery.Params.Items[i].Name;
      Params.Items[i].Value := FQuery.Params.Items[i].Value;
      Params.Items[i].DataType := FQuery.Params.Items[i].DataType;

      ParamValues.Insert(0,FQuery.Params.Items[i].Name);
     end;
    ParamValues.Assign(FParams);
    with TfrDBIParamsForm.Create(nil) do
    begin
      QueryComp := Self;
      Query := FQuery;
      ...
      begin
       for i:=0 to FQuery.ParamCount-1 do
        begin
          FQuery.Params.Items[i].Value := Params.Items[i].Value;
          FQuery.Params.Items[i].DataType := Params.Items[i].DataType;
        end;
...
function TfrDBIQuery.GetParamKind(Index: Integer): TfrParamKind;
begin
  Result := pkValue;
...
procedure TfrDBIQuery.SetParamKind(Index: Integer; Value: TfrParamKind);
begin
...
function TfrDBIQuery.GetParamText(Index: Integer): String;
begin
  Result := '';
...
procedure TfrDBIQuery.SetParamText(Index: Integer; Value: String);
begin
  if ParamKind[Index] = pkValue then
    FParams[FQuery.Params[Index].Name] := Value;
end;

procedure TfrDBIQuery.ReadParams(Stream: TStream);
var
  i: Integer;
  w, n: Word;
begin
...
procedure TfrDBIQuery.WriteParams(Stream: TStream);
var
  i: Integer;
  w: Word;
begin
...
procedure TfrDBIQuery.BeforeOpenQuery(DataSet: TDataSet);
var
  i: Integer;
  SaveView: TfrView;
  SavePage: TfrPage;
...
initialization
  Bmp := TBitmap.Create;
  Bmp.LoadFromResourceName(hInstance, 'FR_DBIQUERYCONTROL');
  frRegisterControl(TfrDBIQuery, Bmp, IntToStr(SInsertQuery));

finalization
  frUnRegisterObject(TfrDBIQuery);
  Bmp.Free;
