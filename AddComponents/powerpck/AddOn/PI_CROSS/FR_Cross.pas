...
type
  ...
  private
    ...
    LastTotalCol: TQuickIntArray;     //!! Массив строк, в которых хранятся колонки, где был Total
    FFlag: Boolean;
    FSkip: Boolean;
    ...
    FReport: TfrReport;
    MaxGTHeight, MaxCellHeight: Integer; //30.11.01 максимальная высота ячейки
    LastX: Integer;                      //!! Хранит последнюю координату для расположение шапки сверху
    DefDY : Integer;                     //!! Dy по умолчанию
    MaxString : String;                  // Максимально длинная строка имени
    LongNames : TStringList;             // имена, длины которых превысили максимум
    ...
    function  GetDataCellText : String;            // Получить текст для ячейки данные
    procedure DictionaryEditor(Sender : TObject);  // Редактор словарика
    function  GetDictName(s : String) : String;    // получить словарный эквивалент имени
    function  CheckLongName(s : String) : String;  // получить имя после проверки на длину
    function  FormatedLongNames : String;          // получить список длинных имен отформатированный для легенды
  protected
  ...
  public
    ...
    FDataWidth  : Integer;                     // -1 - переменная ширина, 0-xxx - постоянная ширина
    FHeaderWidth: Integer;                     // -1 - переменная ширина, 0-xxx - постоянная ширина
    FDictionary: TStringList;                  // словарик
    FMaxNameLen: Integer;                      // Максимальная ширина имени, которая не заменяется *
    FDataCaption: String;                      // Заголовок столбца "Данные"
...
//!! Для русских людей по русски
const GrandTotalStr = {'Grand total'} 'Общий итог';
      TotalStr      = {'Total'}       'Итого';
      TotalOfStr    = {'Total of'}    'Итого по ';
      CellStr       = {'Cell'}        'Показатель';
      HeaderStr     = {'Header'}      'Заголовок';
...
begin
  ...
  OneObject('ColumnHeaderMemo', HeaderStr);                //!! см. ниже

  v := OneObject('ColumnTotalMemo', TotalStr);             //!! см. ниже
  v.FillColor := $F5F5F5;

  v := OneObject('GrandColumnTotalMemo', GrandTotalStr);   //!! см. ниже
  v.FillColor := clSilver;

  OneObject('RowHeaderMemo', HeaderStr);                   //!! см. ниже

  v := OneObject('CellMemo', CellStr);                     //!! см. ниже
  v.Alignment := frtaRight;
  v.Font.Style := [];

  v := OneObject('RowTotalMemo', TotalStr);                //!! см. ниже
  v.FillColor := $F5F5F5;

  v := OneObject('GrandRowTotalMemo', GrandTotalStr);     //!! см. ниже
  v.FillColor := clSilver;
{ комментарии расставлены там, где были заменены стандартные фразы типа GrandTotalRow
  на константы. }  
end;
...
begin
  ...
  AddProperty('ShowHeader', [frdtBoolean], nil);        //!! Добавляем свойство
  AddProperty('ShowGrandTotal', [frdtBoolean], nil);    //30.11.01 Добавляем свойство
  AddProperty('DataWidth', [frdtInteger], nil);         //04.12.01 Добавляем свойство
  AddProperty('HeaderWidth', [frdtInteger], nil);       //04.12.01 Добавляем свойство
...
begin
  ...
  if LVersion > 0 then // 04.01.12 Считываем только если версия добавки подходит
  begin
  ...
  if LVersion > 1 then // 04.01.12 Считываем только если версия добавки подходит
  begin
...
begin
  ...
  m := TMenuItem.Create(Popup);              //!! Добавляем элемент меню
  m.Caption := 'Show header';//LoadStr(SShowHeader);
  m.OnClick := P3Click;
...
begin
  FFlag := True;
  if FDataWidth = -1 then //04.12.01 когда ширина не переменная, то и массив создавать не надо
  ...
  If not FShowGrandTotal then //30.11.01 отключение показа GrandTotal
  begin
    FRowDS.RangeEndCount    := FRowDS.RangeEndCount - 1;
    FColumnDS.RangeEndCount := FColumnDS.RangeEndCount - 1;
  end;

  for k := 0 to FCRoss.CellItemsCount - 1 do //!! Разные показатели в разные ячейки
  begin
    ...
    if FHeaderWidth = -1 then // Header width
    begin
      FColumnDS.First;
      ...
      begin
        ...
        //!! Так правильно, когда больше одного показателя
        If FColumnWidths.Cell[FColumnDS.RecNo] < maxw then FColumnWidths.Cell[FColumnDS.RecNo] := maxw;
        FColumnDS.Next;
      end;
    end;
    if FDataWidth = -1 then //04.12.01 когда ширина не переменная, то рассчитывать ширину не надо
    begin
    ...
    FColumnDS.First;            // расчитываем макс. высоту ячейки по первой строке данных
    while not FColumnDS.EOF do
    begin
      w := v.dx;
      v.dx := 1000;
    ...
    If FShowGrandTotal then     // Если выводим GrandTotal, то рассчитываем его высоту
    begin
  ...
  if MaxCellHeight < DefDy then //30.11.01 сравнили с минимальной
     MaxCellHeight := DefDY;
  if MaxGTHeight < DefDy then   //30.11.01 сравнили с минимальной
     MaxGTHeight := DefDY;
  FFlag  := False;
  LastX := 0;                  //!! Последний X нам не известен
end;
...
begin
  ...
  CalcWidths; //30.11.01 в этой процедуре также считается максимальная высота ячейки

{
 30.11.01 После того как мы рассчитали максимальную высоту ячейки надо перессчитывать
 30.11.01 высоту полоски и высоты ячеек
}
  cd1.dy := MaxCellHeight*FCross.CellItemsCount; //!! повторяем то, что делали выше
  dh := MaxCellHeight;
  d  := cd1.y;
  for i := 0 to FCross.CellItemsCount - 1 do
  begin
  ...
  if FShowHeader then //04.12.01 Для вывода заголовка для строк создаем новые мемо объекты
                      //         и помещаем их на пересечение MasterHeader и CrossHeader
  begin
    d  := ch1.y;
    for i := 0 to FCross.TopLeftSize.cy do
    begin
...
begin
  ...
  begin
  // 04.12.01 изменения связанные с постоянной шириной
...
function GetString(S : String; N : Integer) : String; //!! Получает подстроку из строки Memo
var i : Integer;
begin
  Result := '';
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if s[10] in ['_', '@', '~'] then //!! Новые объекты
        begin
          if s[10] = '@' then //!! i отвечает за номер показателя
            i := StrToInt(Copy(s, 11, 255)) else 
          if s[10] = '~' then //!! ~ отвечает за шапку для строковых измерений
          begin
            Delete(s, 1, 10);
    ...
    end else if View.Name <> 'CrossMemo' + Name then //!! Для разбивки на ячейки
    begin
      s := Copy(View.Name, 1, Pos(Name, View.Name) - 1); 
    ...
    if not FShowHeader and (row = 0) then
      Inc(row);
{ !! Восстанавливаем естественные размеры и видимость объекта, которые изменились
  после коррекций связанных с ColumnTotal, ColumnGrandTotal }
    If not FFlag then
    begin
    ...
    hd := False;
    //!! Тут обрабатывем вне зависимости от колонки, что позволяет нам тут обработать заголовок для строковых измерений
    ...
    begin
      ...
      begin
        If (col >= FCross.TopLeftSize.cx) then
          if (row > 0) then //!! Объединяем пустые мемки с первой левой полной
          begin
        ...
        //!! Печатаем названия строк
          If row = FCross.TopLeftSize.cy then
            View.FrameTyp := 15
          else begin
            v := '';
            if col = FCross.TopLeftSize.cx -1 then View.FrameTyp := frftRight else View.FrameTyp := 0;
            if Col = 0 then inc(View.FrameTyp, frftLeft); // если колонка нулевая, то ограничиваем ее слева
            if (row = 0) then
            begin
              inc(View.FrameTyp, frftTop); // Если нулевая строка, то ограничиваем ее сверху
              if not FCross.DoDataCol then // Если один показатель, то ограничиваем ячейку снизу
      ...
      //!! Не забываем про названия строк даже когда не делаем модификаций
    ...
    begin
      ...
      If (not FFlag) and (row <= FCross.TopLeftSize.cy) then //!! если работаем с шапкой
      begin
        If LastTotalCol.Cell[row-1] < col then //!! Если текущая колонка больше последней обработанной
        begin
          for j := row - 1 to FCross.TopLeftSize.cy do
            LastTotalCol.Cell[j] := Col; //!! фиксируем текущую во все строки шапки начиная с предыдущей
          View.dy := 0;
          for j := row to FCross.TopLeftSize.cy do //!! формируем новый размер
            Inc(View.dy, FColumnHeights.Cell[j]);
        end else View.Visible := False; //!! скрываем ненужные ячейки
      end;
    end
    ...
    begin
{
  Коррекция колонки итого.
  Ранее шапка колонки строилась из нескольких мемо объектов. Теперь из одного, но
  размер этого одного объекта мемо равен суммарному размеру нескольких объеков.
}
...
begin
  ...
  If FShowGrandTotal then //30.11.01 Реакция на непоказ GrandTotal
  begin
    v := FReport.FindObject('GrandColumnTotalMemo' + Name);
    if (v <> nil) and (v.Memo.Count > 0) then
  ...
  FCross.HeaderString := ''; //3.12.01 Так мы выводим в шапке названия измерений по столбцам
