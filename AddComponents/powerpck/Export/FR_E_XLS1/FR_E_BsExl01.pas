...
Type
   ...
   TfrExcelExportFilter01 = class(TfrExportFilter)
   protected
      ...
      FUsePseudographic, FPageBreaks: Boolean;
       ...
       function  GetColumnChar(value: Integer) : String;
...
{ TfrExcelExportFilter01 }

procedure TfrExcelExportFilter01.ClearLines;
var
  i: Integer;
  p: PmyfrView;
begin
...
constructor TfrExcelExportFilter01.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
   frRegisterExportFilter(Self, 'Excel file 01 (*.xls)', '*.xls');
...
destructor TfrExcelExportFilter01.Destroy;
begin
...
function TfrExcelExportFilter01.GetColumnChar(value: Integer): String;
begin
  if Value < 1 then Result:='A'
  else
  begin
    if Value > 702 then
      Result:='ZZ'
    ...
    begin
      if Value > 26 then begin
       if (Value mod 26)=0 then
        Result:=Chr(64 + (Value div 26)-1)
       else
        Result:=Chr(64 + (Value div 26));
         if (Value mod 26)=0 then
          result:=result+chr(64+26)
         else
          result:=Result+Chr(64 + (Value mod 26));
      end
      else
        Result:=Chr(64 + Value);
    end;
...
procedure TfrExcelExportFilter01.OnBeginDoc;
begin
        try
...
procedure TfrExcelExportFilter01.OnBeginPage;
begin
   ClearLines;
end;

procedure TfrExcelExportFilter01.OnData(x, y: Integer; View: TfrView);
Var //p : PfrView ;
      P: PmyfrView;
     I: integer;
//     TfrMemoView
...
procedure TfrExcelExportFilter01.OnEndDoc;
begin
...
procedure TfrExcelExportFilter01.OnEndPage;
var
  i,iMin : Integer;
  P: PmyfrView;
...
begin
  ...
  begin
    ...
    begin
     ...
     s1 := GetColumnChar(FCol)+IntTostr(FRow);
     if p^.dx >= imin *2 then
        s2 := GetColumnChar(FCol+((p^.dx - 5 )div iMin))+IntTostr(FRow)
     else s2 := s1;
        rn1 := FWorksheet.Range[s1,s2];
      rn1.Merge(False);
//      rn1.HorizontalAlignment := xlCenter;
//      rn1.VerticalAlignment := xlCenter;
//    FWorksheet.Range['A1',GetColumnChar(FRow)+'1'].ColumnWidth:= 20;
//    FWorksheet.Range['A1',GetColumnChar(FFieldNames.Count)+'1'].EntireColumn.Autofit;
//    FWorksheet.Range[StrBeginCell,StrEndCell].NumberFormat := '@' //other cases automatic 'general'

      rn1.value := p.Text;
     if(p.FrameTyp and frftTop) <> 0 then
        rn1.Borders.Item[xlEdgeTop].Linestyle := xlSolid;
     if (p.FrameTyp and frftLeft) <> 0 then
        rn1.Borders.Item[xlEdgeLeft].Linestyle := xlSolid;
     if (p.FrameTyp and frftRight) <> 0 then
        rn1.Borders.Item[xlEdgeRight].Linestyle := xlSolid;
     if (p.FrameTyp and frftBottom) <> 0 then
         rn1.Borders.Item[xlEdgeBottom].Linestyle := xlSolid;

      if p.FillColor <> 536870911 then
        rn1.Interior.Color := p.FillColor;

      rn1.EntireRow.Font.Name := p.FontName;
      rn1.EntireRow.Font.Size := p.FontSize;
      rn1.EntireRow.Font.Color:=p.FontColor;
{       if (p.FontStyle = 0 ) then
       ...
       if (p.FontStyle = 0 ) then
         rn1.EntireRow.Font.fontStyle := 'Bold';
       if p.FontStyle = 1 then
         rn1.EntireRow.Font.fontStyle := 'Italic';
       if p.FontStyle = 2 then
         rn1.EntireRow.Font.fontStyle := 'UnderLine';
//frtaLeft, frtaRight, frtaCenter, frtaVertical, frtaMiddle, frtaDown
      case p.Alignment of
       frtaRight : rn1.EntireRow.HorizontalAlignment := xlRight;
       frtaLeft  : rn1.EntireRow.HorizontalAlignment := xlLeft;
       frtaCenter: rn1.EntireRow.HorizontalAlignment := xlCenter;
      end;
      INC(FCol);
