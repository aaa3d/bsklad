...
uses
  ...
  FR_Class, Controls, OleCtnrs, ExtCtrls,
{$IFDEF Delphi2}
  Ole2;
{$ELSE}
  ActiveX;
...
begin
  ...
  frRegisterExportFilter(Self, 'Enchanced RTF (*.rtf)', '*.rtf');
  ShowDialog   := True;
  FExpGraphics := True;
  FExpOLEAsBmp := False;
  FExpFileType := 0;
...
begin
  if not ShowDialog then
    ...
    Result := ShowModal;
    try    FScaleX := frStrToFloat(E1.Text);
    except FScaleX := 1; end;
    try    FScaleY := frStrToFloat(E2.Text);
    except FScaleY := 1; end;
...
var
  ...
  begin
    if (p^.x-prevx)>5 then begin
      s0:=s0+'\clvertalt\cltxlrtb\cellx'+IntToStr(Round(p^.X*ScaleX*TwipsPPixCell));
      s :=s +'\intbl \cell \pard';
    end;
    ...
    s0:=s0
      ...
      + '\cltxlrtb'
      + '\cellx'+IntToStr(Round(p^.DrawRect.Right*ScaleX*TwipsPPixCell));
    s:=s
      + ' \li-95\ri-95'
...
begin
  ...
  for i := 0 to n do begin
    ...
    if (not IsEmpty)and(s<>'')and(s0<>'') then begin
      p := PfrTextRec(Lines[i]);
      if p <> nil then
        s0:=s0+' \trleft'+IntToStr(Round(p^.X * ScaleX * TwipsPPixCell));
      s := '\trowd \trgaph108' +
...
//    if (Memo.Count>0)and(Memo[0]<>'') then if Memo[0][1]<>'0' then Text:='X';
  ...
  if p1 = nil then
    ...
    if p2<>p1
      ...
      else begin Lines[Y]:=TObject(p); p^.Next:=p1; end;
  end;
end;
...
begin
  if not FExpGraphics then begin
    ...
    if TfrRichView(View).RichEdit.Lines.Text<>'' then begin
      Str := TMemoryStream.Create;
      TfrRichView(View).RichEdit.Lines.SaveToStream(Str);
      DataList.Add(Str);
...
begin
{  Caption := LoadStr(frRes + 1840);
  CB1.Caption := LoadStr(frRes + 1841);
  CB2.Caption := LoadStr(frRes + 1842);
  RG1.Caption := LoadStr(frRes + 1845);
  RG1.Items.Clear;
  RG1.Items.Add(LoadStr(frRes + 1846));
  RG1.Items.Add(LoadStr(frRes + 1847));
  RG1.ItemIndex:=0;
  Label1.Caption := LoadStr(frRes + 1806);}
