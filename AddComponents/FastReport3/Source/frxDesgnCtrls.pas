...
uses frxDMPClass, frxPopupForm, frxDsgnIntf, frxCtrls, frxXMLSerializer, Clipbrd;

const
  Colors:Array[0..39] of TColor =
...
begin
  ...
  begin
    ...
    if Align = alLeft then
    begin
      FillRect(Rect(3, FOffset, Width-5, Height));
      fh:= CreateRotatedFont(Font);
      oldfh:= SelectObject(Handle, fh);
    end
    else
      FillRect(Rect(FOffset, 3, Width, Height-5));
  end;
...
begin
  DrawButtonFace(Canvas, BoundsRect, 1, bsNew, False, False, False);
end;
