...
function BreakRussianWord(const s:String):String;
var
  i, j:Integer;
  CanBreak:Boolean;

  function Check1and5(const s:String):Boolean;
  var
    i:Integer;
  begin
    Result:= False;
  ...
  begin
    ...
    begin
      ...
      if (i < Length(s)) and (Pos(s[i+1], glasn)<>0) then
        CanBreak:= False;
      { 3 }
      if (i < Length(s)) and (s[i+1] in ['Ь', 'Ъ']) then
        CanBreak:= False;
    end;
    if Pos(s[i], glasn)<>0 then
    begin
      CanBreak:= True;
      { 4 }
      if (i < Length(s)) and (s[i+1] = 'Й') then
        CanBreak:= False;
      { 6 }
...
procedure TfrxHTMLTagsList.ExpandHTMLTags(var s:String);
var
  i, j, j1:Integer;
  b:Boolean;
  cl:String;
  ...
  begin
    ...
    if FAllowTags then
      ...
      begin
        ...
        // <strike> tag
        else if (i+1 <= Length(s)) and (s[i+1] in ['s', 'S']) then
        begin
...
function TfrxHTMLTagsList.FillCharSpacingArray(var ar:PIntArray; const s:String;
  Canvas:TCanvas; LineIndex, Add:Integer; Convert:Boolean):Integer;
var
  i, n:Integer;
  Tags:TfrxHTMLTags;
  ...
  var
    ...
    Size:TSize;
  begin
    GetTextExtentExPoint(Canvas.Handle, PChar(s), n, 0, nil,
      @FTempArray[1], Size);
    i:= 0;
    repeat
      if FTempArray[i+1] = 32767 then
      begin
        offs:= FTempArray[i];
        GetTextExtentExPoint(Canvas.Handle, PChar(s)+i, n-i, 0, nil,
          @FTempArray[i+1], Size);
        for j:= i+1 to n do
...
begin
  ...
  begin
    Tag:= Tags.Items[i-1];
    if (i<>1) and not Tag.Default then
    begin
      Canvas.Font.Style:= Tag.Style;
      BreakArray;
    end;
...
begin
  ...
  FCanvas:= FBMP.Canvas;
  FDefPPI:= 600;
  FScrPpi:= 96;//GetDeviceCaps(FCanvas.Handle, LOGPIXELSY);
...
procedure TfrxDrawText.SetOptions(WordWrap, HTMLTags, RTLReading,
  WordBreak, Clipped:Boolean; Rotation:Integer);
begin
  FWordWrap:= WordWrap;
...
procedure TfrxDrawText.SetDimensions(ScaleX, ScaleY:Extended; OriginalRect,
  ScaledRect:TRect);
begin
  FScaleX:= ScaleX;
  FScaleY:= ScaleY;
...
procedure TfrxDrawText.SetText(Text:TStrings);
var
  i, j, n, Width:Integer;
  s:String;
  Style:TFontStyles;
...
// width of the wrap area
  ...
  begin
    ...
    WrapTextLine(s,
      Round(Width * FDefPPI / FScrPPI),
      Round((Width-FParagraphGap) * FDefPPI / FScrPPI),
      Round(FCharSpacing * FDefPPI / FScrPPI));
...
function TfrxDrawText.DeleteTags(const Txt:String):String;
begin
  Result:= Txt;
...
procedure TfrxDrawText.DrawTextLine(C:TCanvas; const s:String;
  X, Y, DX, LineIndex:Integer; Align:TfrxHAlign; var fh, oldfh:HFont);
var
  spaceAr:PIntArray;
...
begin
  ...
  PrnSz:= FHTMLTags.FillCharSpacingArray(FTempArray, s, FCanvas, LineIndex,
    Round(FCharSpacing * FDefPPI / FScrPPI), False)-
    Round(FCharSpacing * FDefPPI / FScrPPI);
  ...
  ratio:= FDefPPI / PPI;
  if IsPrinter(C) then
    neededSize:= Round(prnSz / ratio) else
  ...
  begin
    if extraSize < n then
      IncArray(FTempArray, 1, n, extraSize, add3)
    else
    begin
      add1:= extraSize div n * add3;
      for i:= 1 to n do
        Inc(FTempArray[i], add1);
      IncArray(FTempArray, 1, n, extraSize-add1 * n * add3, add3)
    end;
  end
  else
  begin
  ...
  repeat
    ...
    ExtTextOut(C.Handle, X+Round(add1 * SinA), Y+Round(add1 * CosA),
      FOptions, @FScaledRect, PChar(s)+j, i-j, @FTempArray[j+1]);

    if i < n then
    begin
      if IsPrinter(C) then
        C.Font.Height:=-Round(Tag.Size * PPI / 72) else
        C.Font.Height:=-Round(Tag.Size * FScaleY * 96 / 72);
      C.Font.Style:= Tag.Style;
      C.Font.Color:= Tag.Color;
      add1:= Round(Tag.AddY * Tag.Size * FScaleY);

      cw:= CalcWidth(j+1, i-j);
      if FRotation = 0 then
        X:= X+cw
      else
      begin
...
begin
  ...
  if n > 1 then
    if extraSize < n then
      IncArray(Ar, 1, n-1, extraSize, add3)
    else if n > 1 then
    begin
      add1:= extraSize div (n-1) * add3;
      for i:= 1 to n-1 do
        Inc(Ar[i], add1);
      IncArray(Ar, 1, n-1, extraSize-add1 * (n-1) * add3, add3)
    end;
  ...
  begin
    ...
    Inc(y, Round(Ar[i+1] * CosA));
    Inc(x, Round(Ar[i+1] * SinA));
  end;
...
function TfrxDrawText.GetOutBoundsText:String;
var
  PrnSz:Integer;
  n, vl:Integer;
  ratio:Extended;
...
function TfrxDrawText.GetInBoundsText:String;
begin
  Result:= FPlainText;
end;
