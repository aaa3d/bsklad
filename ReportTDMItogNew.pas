...
type
  TREPORT_TDM_ITOG_NEW = class(TForm)
...
var
  REPORT_TDM_ITOG_NEW: TREPORT_TDM_ITOG_NEW;
  RNTYP_S:ansistring;
  PNTYP_S:ansistring;
  REPORT_TITLE:string;
...
procedure TREPORT_TDM_ITOG_NEW.BtnOkClick(Sender: TObject);
var
  dt1,dt2:tdatetime;
  pp_typs: string;
begin
   REPORT_TITLE:='Новый итоговый отчет об остатках товаров по фирмам на конец '+
...
procedure TREPORT_TDM_ITOG_NEW.FormShow(Sender: TObject);
var
  s3,s4:string;
  a:integer;
begin
...
procedure TREPORT_TDM_ITOG_NEW.BtnDicClick(Sender: TObject);
begin
...
procedure TREPORT_TDM_ITOG_NEW.frReport1GetValue(const ParName: String;
  var ParValue: Variant);
begin
...
procedure TREPORT_TDM_ITOG_NEW.BtnPrintClick(Sender: TObject);
begin
     FrReport1.ShowReport;
end;

procedure TREPORT_TDM_ITOG_NEW.MesChange(Sender: TObject);
begin
    label_mes1.caption:='месяц/месяца/месяцев назад';

end;

procedure TREPORT_TDM_ITOG_NEW.ExpExcel;
...
//    excel.Visible[0]:=true;
    ...
    excel.Workbooks.Add(ExtractFilePath(ParamStr(0))+CN_TemplateFolder+'\отчет об остатках по фирмам.xlt', 0);
    ...
    r:= excel.Range['A1', 'A1'];
    r.Value := 'Новый итоговый отчет об остатках по фирмам на конец ' + dt.Text;
    r:= excel.Range['A2', 'A2'];
...
procedure TREPORT_TDM_ITOG_NEW.BtnDynDicClick(Sender: TObject);
begin
...
procedure TREPORT_TDM_ITOG_NEW.ExpCalc;
...
// Открытие CALC
   ...
   filePath:=convertToURL(ExtractFilePath(ParamStr(0))+CN_TemplateFolder+'\отчет об остатках по фирмам.xlt');
...
procedure TREPORT_TDM_ITOG_NEW.btnExcelClick(Sender: TObject);
var excel: TExcelApplication;
begin
  try
    Excel := TExcelApplication.Create(nil);
...
procedure TREPORT_TDM_ITOG_NEW.DYNPOSTClick(Sender: TObject);
begin
     try
...
procedure TREPORT_TDM_ITOG_NEW.BTN_POSTClick(Sender: TObject);
var fr:integer;
begin
