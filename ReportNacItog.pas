...
var
  REPORT_NAC_ITOG: TREPORT_NAC_ITOG;
  RNTYP_S:ansistring;
  PNTYP_S:ansistring;
  REPORT_TITLE:string;
...
procedure TREPORT_NAC_ITOG.BtnOKClick(Sender: TObject);
var
  PTYP,rntypS,pntypS: string;
begin
   REPORT_TITLE:='Итоговый отчет о наценке по менеджерам за период с '+
   ...
   rntypS:=RNTYP_S;
   pntypS:=PNTYP_S;
   ADD_SQL(QueryRep1, 'select men, typ, sum(seb*tw_kol) seb,   sum(nac) nac '+
...
procedure TREPORT_NAC_ITOG.FormShow(Sender: TObject);
var
  s3,s4:string;
  a:integer;
begin
...
procedure TREPORT_NAC_ITOG.BtnDicClick(Sender: TObject);
begin
...
procedure TREPORT_NAC_ITOG.SpeedButton1Click(Sender: TObject);
var
   a:integer;
   s,s2:ansistring;
begin
...
procedure TREPORT_NAC_ITOG.SpeedButton2Click(Sender: TObject);
var
   a:integer;
   s,s2:ansistring;
begin
...
procedure TREPORT_NAC_ITOG.frReport1GetValue(const ParName: String;
  var ParValue: Variant);
begin
...
procedure TREPORT_NAC_ITOG.BtnPrintClick(Sender: TObject);
begin
     FrReport1.ShowReport;
end;

procedure TREPORT_NAC_ITOG.ExpExcel;
...
begin
    ...
    excel.Workbooks.Add(ExtractFilePath(ParamStr(0))+CN_TemplateFolder+'\отчет по наценке по менеджерам.xlt', 0);
    ...
    r:= excel.Range['A1', 'A1'];
    r.Value := 'Итоговый отчет о наценке по менеджерам за период с ' + dt1.Text +' по ' + dt2.Text;
    r:= excel.Range['A2', 'A2'];
...
//    r.AutoFit;
    ...
    format := MemReportseb.DisplayFormat;
    format := stringreplace(format, ' ', '', [rfReplaceAll]);
    format := stringreplace(format, '.', DECIMALSEPARATOR, [rfReplaceAll]);
    r.NumberFormat := format;
...
procedure TREPORT_NAC_ITOG.BtnDynDicClick(Sender: TObject);
begin
...
procedure TREPORT_NAC_ITOG.ExpCalc;
...
procedure TREPORT_NAC_ITOG.btnExcelClick(Sender: TObject);
var excel: TExcelApplication;
begin
  try
    Excel := TExcelApplication.Create(nil);
