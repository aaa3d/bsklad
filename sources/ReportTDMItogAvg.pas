...
type
  TREPORT_TDM_ITOG_AVG = class(TForm)
    ReportTransaction: TIBTransaction;
    QueryRep1: TIBQuery;
    BtnOk: TButton;
    MemReport: TRxMemoryData;
    MemReportMEN_ID: TIntegerField;
    MemReportMEN_FULL: TStringField;
...
var
  REPORT_TDM_ITOG_AVG: TREPORT_TDM_ITOG_AVG;
  RNTYP_S:ansistring;
  PNTYP_S:ansistring;
  REPORT_TITLE:string;
...
procedure TREPORT_TDM_ITOG_AVG.BtnOkClick(Sender: TObject);
var
  dt1,dt2:tdatetime;
  pp_typs: string;
begin
   REPORT_TITLE:='Ќовый итоговый отчет об остатках товаров по менеджерам на конец '+
   ...
   ADD_SQL(QueryRep1,'select men_id,valuta_id,sum(ost_kol*seb) as ost,sum(ost_kol*seb$) as ost$ '+
   'from report_itog_tdm_new(:dt1,:dt2,:dtrep,:twtree_id, :pp_typs, :twtree_typ) group by men_id,valuta_id order by men_id');
   ...
   while not QueryRep1.eof do begin
     MemReport.Append;
     MemReport.fieldByname('MEN_ID').asinteger:=QueryRep1.fieldByname('men_id').asinteger;
     ADD_SQL(QueryRep2,'select kln_full from kln_data(:kln_id)');
     QueryRep2.ParamByname('kln_id').asinteger:=QueryRep1.fieldByname('men_id').asinteger;
     QueryRep2.Open;
     MemReport.fieldByname('men_full').asstring:=QueryRep2.fieldByname('kln_full').asstring;
...
procedure TREPORT_TDM_ITOG_AVG.FormShow(Sender: TObject);
var
  s3,s4:string;
  a:integer;
begin
...
procedure TREPORT_TDM_ITOG_AVG.BtnDicClick(Sender: TObject);
begin
...
procedure TREPORT_TDM_ITOG_AVG.frReport1GetValue(const ParName: String;
  var ParValue: Variant);
begin
...
procedure TREPORT_TDM_ITOG_AVG.BtnPrintClick(Sender: TObject);
begin
     FrReport1.ShowReport;
end;

procedure TREPORT_TDM_ITOG_AVG.MesChange(Sender: TObject);
begin
...
procedure TREPORT_TDM_ITOG_AVG.btnExcelClick(Sender: TObject);
...
//    excel.Visible[0]:=true;
    ...
    excel.Workbooks.Add(ExtractFilePath(ParamStr(0))+CN_TemplateFolder+'\отчет об остатках по менеджерам.xlt', 0);
    ...
    r:= excel.Range['A1', 'A1'];
    r.Value := 'Ќовый итоговый отчет об остатках по менеджерам на конец ' + dt.Text;
    r:= excel.Range['A2', 'A2'];
...
procedure TREPORT_TDM_ITOG_AVG.BtnDynDicClick(Sender: TObject);
begin
