...
type
  TMAKE_TW_REAL_TO_KLN = class(TForm)
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
...
var
  MAKE_TW_REAL_TO_KLN: TMAKE_TW_REAL_TO_KLN;
...
procedure TMAKE_TW_REAL_TO_KLN.BtnTWClick(Sender: TObject);
begin
...
procedure TMAKE_TW_REAL_TO_KLN.BtnDynDicClick(Sender: TObject);
begin
...
procedure TMAKE_TW_REAL_TO_KLN.TW1Click(Sender: TObject);
begin
   if TW1.Checked=true
     then
     begin
...
procedure TMAKE_TW_REAL_TO_KLN.Button1Click(Sender: TObject);
begin
  datamodule1.IbSaveSQL.Close;

  datamodule1.IbSaveSQL.SQL.Text:='SELECT NUM_FULL FROM A2_MAKE_REAL_TO_KLN(:dt, :frm_id, :kln_id, :skl_id, :shop_id, :TW_GROUP_TYP, :TW_GROUP_ID)';


  datamodule1.IbSaveSQL.ParamByName('FRM_ID').asstring:=FRM.Value;
  datamodule1.IbSaveSQL.ParamByName('KLN_ID').asstring:=POST.Value;
  datamodule1.IbSaveSQL.ParamByName('dt').asdatetime:=round(dt1.Date) + frac(tm1.time);
...
procedure TMAKE_TW_REAL_TO_KLN.FormCreate(Sender: TObject);
begin
