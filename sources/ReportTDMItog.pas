...
var
  REPORT_TDM_ITOG: TREPORT_TDM_ITOG;
  RNTYP_S:ansistring;
  PNTYP_S:ansistring;
  REPORT_TITLE:string;
...
procedure TREPORT_TDM_ITOG.BtnOkClick(Sender: TObject);
var
  dt1,dt2:tdatetime;
  pp_typs: string;
begin
   REPORT_TITLE:='»тоговый отчет об остатках товаров по менеджерам на конец '+
   ...
   'from report_itog_tdm(:dt1,:dt2,:dtrep,:twtree_id, :pp_typs, :twtree_typ) group by men_id,valuta_id order by men_id');
...
procedure TREPORT_TDM_ITOG.FormShow(Sender: TObject);
var
  s3,s4:string;
  a:integer;
begin
...
procedure TREPORT_TDM_ITOG.BtnDicClick(Sender: TObject);
begin
...
procedure TREPORT_TDM_ITOG.frReport1GetValue(const ParName: String;
  var ParValue: Variant);
begin
...
procedure TREPORT_TDM_ITOG.BtnPrintClick(Sender: TObject);
begin
     FrReport1.ShowReport;
end;

procedure TREPORT_TDM_ITOG.MesChange(Sender: TObject);
begin
...
procedure TREPORT_TDM_ITOG.btnExcelClick(Sender: TObject);
...
//    excel.Visible[0]:=true;
    ...
    r:= excel.Range['A1', 'A1'];
    r.Value := '»тоговый отчет об остатках по менеджерам на конец ' + dt.Text;
    r:= excel.Range['A2', 'A2'];
...
procedure TREPORT_TDM_ITOG.BtnDynDicClick(Sender: TObject);
begin
