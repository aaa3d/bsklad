...
type
  TREPORT_SKLAD_WORK = class(TForm)
    ...
    QueryRep1PERIOD_SUM: TFloatField;
    chk_remove_palet_weight: TCheckBox;
    QueryRep1PERIOD_PALET: TFloatField;
    chk_show_firms: TCheckBox;
    QueryRep1KOR1: TFloatField;
    QueryRep1KOR2: TFloatField;
    QueryRep1RASH_KOR: TFloatField;
    QueryRep1POST_KOR: TFloatField;
    QueryRep1PER_KOR: TFloatField;
    QueryRep1PERIOD_KOR: TFloatField;
    chkUseKor: TCheckBox;
    QueryRep1PERIOD_KOR_KM: TFloatField;
    QueryRep1ED1_KM: TFloatField;
    QueryRep1ED2_KM: TFloatField;
    QueryRep1RASH_ED_KM: TFloatField;
    QueryRep1POST_ED_KM: TFloatField;
    QueryRep1PER_ED_KM: TFloatField;
    QueryRep1PERIOD_ED_KM: TFloatField;
    QueryRep1RASH_WEIGHT_NEGAB: TFloatField;
    QueryRep1POST_WEIGTH_NEGAB: TFloatField;
    QueryRep1PER_WEIGHT_NEGAB: TFloatField;
    QueryRep1PERIOD_NETIP_PALET: TFloatField;
    chk_split_tw: TCheckBox;
...
{   ADD_SQL(QueryRep1,'select tw_id,tw_art,tw_nam,sum(ost_kol) as ost_kol, '+
   ...
   QueryRep1.Transaction.Active:=false;
   ...
   begin
     ...
     REPORT_TITLE:=REPORT_TITLE+', по закупочным ценам';
   end;
   QueryRep1.ParamByname('USE_PALET').asinteger:=0;
   QueryRep1.ParamByname('USE_KOR').asinteger:=0;
   if chkUsePalet.Checked then
   begin
     QueryRep1.ParamByname('USE_PALET').asinteger:=1;
     REPORT_TITLE:=REPORT_TITLE+', рассчет по палетам';
   end;
   if chkUseKor.Checked then
   begin
     QueryRep1.ParamByname('USE_KOR').asinteger:=1;
     REPORT_TITLE:=REPORT_TITLE+', рассчет по коробкам';
   end;

   QueryRep1.ParamByname('SHOW_FIRMS').asinteger:=0;
   if chk_show_firms.Checked then
   begin
     QueryRep1.ParamByname('SHOW_FIRMS').asinteger:=1;
     REPORT_TITLE:=REPORT_TITLE+', по фирмам';
   end;

   QueryRep1.ParamByname('split_tw').asinteger:=0;
   if chk_split_tw.Checked then
   begin
     QueryRep1.ParamByname('split_tw').asinteger:=1;
     REPORT_TITLE:=REPORT_TITLE+',  разделением легких/тяжелых товаров';
   end;


{
   if self.chk_remove_palet_weight.Checked then
   begin
     QueryRep1.ParamByname('remove_palet_weight').asinteger:=1;
     REPORT_TITLE:=REPORT_TITLE+', с вычитанием массы паллет';
   end;
}
...
//    excel.Visible[0]:=true;
    ...
    excel.Workbooks.Add(ExtractFilePath(ParamStr(0))+CN_TemplateFolder+'\отчет о работе склада.xlt', 0);
