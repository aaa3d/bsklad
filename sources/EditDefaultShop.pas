...
uses
  ...
  IBCustomDataSet, IBQuery, IBDatabase, RxLookup;

type
  TEDIT_DEFAULT_SHOP = class(TForm)
    ToolBar1: TToolBar;
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
...
var
  EDIT_DEFAULT_SHOP: TEDIT_DEFAULT_SHOP;
  SAVE:boolean;
...
procedure TEDIT_DEFAULT_SHOP.Btn_FRMClick(Sender: TObject);
begin
...
procedure TEDIT_DEFAULT_SHOP.BTN_KLNClick(Sender: TObject);
begin
     DicKlnSet:=2; {Выбор клиента}
     Application.createForm(TDIC_KLIENT,DIC_KLIENT);
...
procedure TEDIT_DEFAULT_SHOP.BTN_SAVEClick(Sender: TObject);
begin
     tag:=2;
     SAVE:=true;
     close;
...
procedure TEDIT_DEFAULT_SHOP.FormShow(Sender: TObject);
begin
     tag:=0;
     Change_FrmKln(KLN.tag,1);
     Change_FrmKln(FRM.tag,2);
...
procedure TEDIT_DEFAULT_SHOP.Change_FrmKln(ID,N:integer);
var
   s1,s2:ansistring;
begin
...
procedure TEDIT_DEFAULT_SHOP.FormKeyPress(Sender: TObject; var Key: Char);
begin
     if key=#27 then close;
end;

procedure TEDIT_DEFAULT_SHOP.FormCloseQuery(Sender: TObject;
...
procedure TEDIT_DEFAULT_SHOP.ToolButton1Click(Sender: TObject);
begin
     My_lock;
end;

procedure TEDIT_DEFAULT_SHOP.RVIDChange(Sender: TObject);
begin
        SAVE:=False;
end;
