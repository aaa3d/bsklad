     ...
     begin
      WorkTransaction.Active:=False;
   	  ...
   	  ADD_SQL(QueryWork2,'select sum(rns.tw_kol*roundd((rns.tw_money*(1-rns.tw_skiddoll/100))*(1-(rns.tw_skid+rn.rn_skid)/100),2)) rn_sum '+
						 'from rn, rntyp, rns where rn.rn_prow=1 and rns.rn_id = rn.rn_id and rn.rn_date > :dt1 and rn.rntyp_id = rntyp.rntyp_id and rntyp.real_typ=1 and rn.kln_id = :kln_id and rn.frm_id in (:frm_id, :frm_id1, :frm_id2)');
      QueryWork2.ParamByname('KLN_ID').asinteger:=Edit_Kontakt.tag;
      QueryWork2.ParamByname('dt1').asdatetime:=saldo_date;
      if gl_FRM_WORK_MODE=0 then
      begin
          QueryWork2.Parambyname('FRM_ID').asinteger:=2702542;
          QueryWork2.Parambyname('FRM_ID1').asinteger:=2669809;
